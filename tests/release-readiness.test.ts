...
const readme = "> Published package: v0.3.0. Context Autopilot is the default lifecycle.";
...
describe("v0.3 release readiness", () => {
  it("passes a complete v0.3.0 release candidate", () => {
  ...
  it("fails version, docs, smoke, and package privacy drift independently", () => {
    const result = evaluateReleaseReadiness({
      package_json: { ...packageJson, version: "0.3.1" },
      readme,
      changelog: "# Changelog\n\n## 0.2.0\n",
      release_check_source: "official_host_handoff_smoke",
      packed_files: [...packedFiles, "docs/releases/v0.3-roadmap.md", ".codex/session.json"]
    });
    ...
    expect(result.checks).toMatchObject({
      ...
      private_paths_absent: {
        status: "failed",
        offending: ["docs/releases/v0.3-roadmap.md", ".codex/session.json"]
      }
    });
  });
});
