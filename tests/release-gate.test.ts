...
describe("v0.3 release gate", () => {
  it("defines one ordered gate containing tests, both smokes, package validation, and optional remote validation", () => {
    expect(releaseGateSteps(false, true).map((step) => step.id)).toEqual(["build", "typecheck", "tests", "release_readiness", "dogfood_smoke", "lifecycle_smoke", "learning_inbox_smoke", "finalization_assurance_smoke", "host_runtime_binding_smoke", "transcript_compact_safety_smoke", "official_host_handoff_smoke", "upgrade_compat_smoke", "sync_resilience_smoke", "sync_conflict_smoke", "permission_recovery_smoke", "large_store_smoke", "package", "private_remote"]);
    expect(releaseGateSteps(true, false).map((step) => [step.id, step.mode])).toEqual([
      ["build", "skipped"], ["typecheck", "skipped"], ["tests", "skipped"], ["release_readiness", "required"], ["dogfood_smoke", "required"], ["lifecycle_smoke", "required"], ["learning_inbox_smoke", "required"], ["finalization_assurance_smoke", "required"], ["host_runtime_binding_smoke", "required"], ["transcript_compact_safety_smoke", "required"], ["official_host_handoff_smoke", "required"], ["upgrade_compat_smoke", "required"], ["sync_resilience_smoke", "required"], ["sync_conflict_smoke", "required"], ["permission_recovery_smoke", "required"], ["large_store_smoke", "required"], ["package", "required"], ["private_remote", "optional_skipped"]
    ]);
  });
  ...
  it("emits machine-readable evidence only after every required step succeeds", async () => {
    const calls: string[] = [];
    const logs: string[] = [];
    const result = await runReleaseGate({ skip_slow_checks: true, private_remote: undefined, run_command: async (command, args) => { calls.push([command, ...args].join(" ")); return command === "npm" && args[0] === "pack" ? JSON.stringify([{ files: [{ path: "package.json" }, { path: "LICENSE" }, { path: "README.md" }, { path: "CHANGELOG.md" }, { path: "docs/agent-install-prompt.md" }, { path: "docs/agent-workflow.md" }, { path: "docs/contracts.md" }, { path: "docs/development.md" }, { path: "docs/implementation-roadmap.md" }, { path: "docs/moryn-design.md" }, { path: "dist/cli.js" }, { path: "dist/index.js" }, { path: "dist/mcp/server.js" }, { path: "scripts/agent-lifecycle-smoke.js" }, { path: "scripts/host-runtime-binding-smoke.js" }, { path: "scripts/transcript-compact-safety-smoke.js" }, { path: "scripts/official-host-handoff-smoke.js" }, { path: "scripts/learning-inbox-smoke.js" }, { path: "scripts/finalization-assurance-smoke.js" }, { path: "scripts/dogfood-demo-smoke.js" }, { path: "scripts/upgrade-compat-smoke.js" }, { path: "scripts/sync-resilience-smoke.js" }, { path: "scripts/sync-conflict-smoke.js" }, { path: "scripts/permission-recovery-smoke.js" }, { path: "scripts/large-store-smoke.js" }] }]) : "ok"; }, log: (line) => logs.push(line) });
    expect(calls).toEqual(["npm run release:readiness", "npm run smoke:dogfood-demo", "npm run smoke:agent-lifecycle", "npm run smoke:learning-inbox", "npm run smoke:finalization-assurance", "npm run smoke:host-runtime-binding", "npm run smoke:transcript-compact-safety", "npm run smoke:official-host-handoff", "npm run smoke:upgrade-compat", "npm run smoke:sync-resilience", "npm run smoke:sync-conflict", "npm run smoke:permission-recovery", "npm run smoke:large-store", "npm pack --dry-run --json"]);
    expect(result).toMatchObject({ version: 1, status: "passed", completed: ["release_readiness", "dogfood_smoke", "lifecycle_smoke", "learning_inbox_smoke", "finalization_assurance_smoke", "host_runtime_binding_smoke", "transcript_compact_safety_smoke", "official_host_handoff_smoke", "upgrade_compat_smoke", "sync_resilience_smoke", "sync_conflict_smoke", "permission_recovery_smoke", "large_store_smoke", "package"], skipped: ["build", "typecheck", "tests", "private_remote"], acceptance_complete: false });
  ...
  it("maps a full release run to all nine v0.3 acceptance areas", () => {
    ...
    expect(acceptance.learning.required_evidence).toEqual(expect.arrayContaining(["learning_inbox_smoke", "finalization_assurance_smoke"]));
