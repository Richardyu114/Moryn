...
describe("v0.3 release gate", () => {
  it("defines one ordered gate containing tests, both smokes, package validation, and optional remote validation", () => {
    expect(releaseGateSteps(false, true).map((step) => step.id)).toEqual(["build", "typecheck", "tests", "dogfood_smoke", "lifecycle_smoke", "host_runtime_binding_smoke", "transcript_compact_safety_smoke", "upgrade_compat_smoke", "sync_resilience_smoke", "sync_conflict_smoke", "permission_recovery_smoke", "large_store_smoke", "package", "private_remote"]);
    expect(releaseGateSteps(true, false).map((step) => [step.id, step.mode])).toEqual([
      ["build", "skipped"], ["typecheck", "skipped"], ["tests", "skipped"], ["dogfood_smoke", "required"], ["lifecycle_smoke", "required"], ["host_runtime_binding_smoke", "required"], ["transcript_compact_safety_smoke", "required"], ["upgrade_compat_smoke", "required"], ["sync_resilience_smoke", "required"], ["sync_conflict_smoke", "required"], ["permission_recovery_smoke", "required"], ["large_store_smoke", "required"], ["package", "required"], ["private_remote", "optional_skipped"]
    ]);
  });
  ...
  it("maps a full release run to all nine v0.3 acceptance areas", () => {
    ...
    expect(acceptance.hosts.required_evidence).toEqual(expect.arrayContaining(["tests", "lifecycle_smoke", "host_runtime_binding_smoke", "transcript_compact_safety_smoke"]));
  });
