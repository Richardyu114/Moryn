...
describe("v0.3 release gate", () => {
  ...
  it("emits machine-readable evidence only after every required step succeeds", async () => {
    ...
    expect(calls).toEqual(["npm run smoke:dogfood-demo", "npm run smoke:agent-lifecycle", "npm run smoke:learning-inbox", "npm run smoke:finalization-assurance", "npm run smoke:host-runtime-binding", "npm run smoke:transcript-compact-safety", "npm run smoke:official-host-handoff", "npm run smoke:upgrade-compat", "npm run smoke:sync-resilience", "npm run smoke:sync-conflict", "npm run smoke:permission-recovery", "npm run smoke:large-store", "npm pack --dry-run --json"]);
    expect(result).toMatchObject({ version: 1, status: "passed", completed: ["dogfood_smoke", "lifecycle_smoke", "learning_inbox_smoke", "finalization_assurance_smoke", "host_runtime_binding_smoke", "transcript_compact_safety_smoke", "official_host_handoff_smoke", "upgrade_compat_smoke", "sync_resilience_smoke", "sync_conflict_smoke", "permission_recovery_smoke", "large_store_smoke", "package"], skipped: ["build", "typecheck", "tests", "private_remote"], acceptance_complete: false });
