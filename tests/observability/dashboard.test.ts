...
describe("observability dashboard", () => {
  ...
  it("adds a read-only candidate triage queue for memory doctor backlog", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(html).toContain("<strong>Audit only</strong>");
  ...
  it("keeps candidate triage samples budgeted while preserving full API evidence", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(groupHtml).toContain("<strong>Audit only</strong>");
  ...
  it("redacts quarantined record text and escapes rendered HTML", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(html).toContain("<h3>Evidence index</h3>");
      expect(html).toContain("<div class=\"evidence-library-routebar\" role=\"list\" aria-label=\"Evidence index\">");
      expect(html).not.toContain("<h3>Evidence map</h3>");
      expect(html).not.toContain("<h3>Evidence routes</h3>");
      expect(html).not.toContain("<div class=\"evidence-library-brief-grid\">");
      expect(html).not.toContain("class=\"evidence-library-route\" data-evidence-library-route=\"findings\"");
      ...
      expect(candidateTriageHtml).toContain("<strong>Audit only</strong>");
  ...
  it("renders maintenance review queue controls", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(html).toContain("<span>Audit details</span>");
      expect(html).toContain("<small>Decision record and confirmation</small>");
      ...
      expect(html).toContain("<details class=\"maintenance-confirm-notes\" data-dashboard-detail=\"maintenance-confirm-notes:");
      expect(html).toContain("<span>Confirm notes</span>");
      expect(html).toContain("<small>Checklist before approval</small>");
      ...
      const confirmNotesStart = html.indexOf("<details class=\"maintenance-confirm-notes\" data-dashboard-detail=\"maintenance-confirm-notes:", auditDetailsStart);
      const approvalChecklistStart = html.indexOf("<details class=\"approval-checklist\" data-dashboard-detail=\"maintenance-approval-checklist:", auditDetailsStart);
      ...
      expect(confirmNotesStart).toBeGreaterThan(auditDetailsStart);
      expect(approvalChecklistStart).toBeGreaterThan(confirmNotesStart);
      expect(planEvidenceStart).toBeGreaterThan(auditDetailsStart);
      expect(planEvidenceStart).toBeGreaterThan(approvalChecklistStart);
