...
describe("observability dashboard", () => {
  ...
  it("reports configured dirty sync as Sync Pending instead of generic review", async () => {
    ...
    try {
      ...
      expect(referenceIndexFaceHtml).toContain("<strong data-i18n-en=\"Saved details\" data-i18n-zh=\"保存细节\">Saved details</strong>");
      expect(referenceIndexFaceHtml).not.toContain("<strong>Audit Index</strong>");
      expect(referenceIndexFaceHtml).toContain("<span data-i18n-en=\"Read-only details available\" data-i18n-zh=\"可查看只读详情\">Read-only details available</span>");
      expect(referenceIndexFaceHtml).toContain("<small data-i18n-en=\"Optional context\" data-i18n-zh=\"可选上下文\">Optional context</small>");
      expect(referenceIndexFaceHtml).not.toContain("<strong>Reference Library Index</strong>");
      expect(referenceRoutesHtml).toContain("<span data-i18n-en=\"Detail links\" data-i18n-zh=\"详情入口\">Detail links</span>");
      expect(referenceRoutesHtml).toContain("<small data-i18n-en=\"Routes and checks\" data-i18n-zh=\"入口和检查\">Routes and checks</small>");
      ...
      expect(referenceRoutesHtml).toContain("<code data-reference-library-route=\"routine-diagnostics\" data-i18n-en=\"Health checks\" data-i18n-zh=\"健康检查\">Health checks</code>");
      ...
      expect(referenceRoutesHtml).toContain("<strong data-i18n-en=\"Health Checks\" data-i18n-zh=\"健康检查\">Health Checks</strong>");
      expect(referenceRoutesHtml).not.toContain("<strong>Diagnostics Index</strong>");
      expect(referenceRoutesHtml).toContain("<code data-dashboard-detail=\"health-check\" aria-label=\"Health Check: Healthy local store. Full report is available in /api/dashboard.health_check.\" data-i18n-en=\"Health check\" data-i18n-zh=\"健康检查\">Health check</code>");
      expect(referenceRoutesHtml).toContain("<code data-dashboard-detail=\"recall-eval\" aria-label=\"Recall Eval: No recall eval cases yet. Full report is available in /api/dashboard.recall_eval.\" data-i18n-en=\"Recall check\" data-i18n-zh=\"召回检查\">Recall check</code>");
      expect(referenceRoutesHtml).not.toContain(">health_check</code>");
      expect(referenceRoutesHtml).not.toContain(">recall_eval</code>");
      expect(referenceRoutesHtml).toContain("<span data-i18n-en=\"Routine checks indexed\" data-i18n-zh=\"日常检查已建立索引\">Routine checks indexed</span>");
  ...
  it("renders a human-first dashboard with memory inventory, shared copy, recent status, and language switch", async () => {
    ...
    try {
      ...
      expect(data.memory_inventory.states.map((state) => state.label)).toEqual(["Long-term", "Recently saved", "For this session", "Kept for history"]);
      ...
      expect(html).toContain("<span data-i18n-en=\"Detail links\" data-i18n-zh=\"详情入口\">Detail links</span>");
      expect(html).toContain("<small data-i18n-en=\"Routes and checks\" data-i18n-zh=\"入口和检查\">Routes and checks</small>");
      ...
      expect(html).toContain("<span class=\"answer-memory-segment memory-state-remembered\" style=\"width: 33%\" title=\"Long-term 1\"></span>");
      expect(html).toContain("<span class=\"answer-memory-segment memory-state-to-organize\" style=\"width: 33%\" title=\"Recently saved 1\"></span>");
      expect(html).toContain("<span class=\"answer-memory-segment memory-state-temporary\" style=\"width: 33%\" title=\"For this session 1\"></span>");
      expect(html).toContain("<div class=\"answer-memory-counts\" data-answer-memory-counts>");
      expect(html).toContain("<span data-i18n-en=\"1 long-term\" data-i18n-zh=\"1 条长期记住\">1 long-term</span>");
      expect(html).toContain("<span data-i18n-en=\"1 recently saved\" data-i18n-zh=\"1 条最近保存\">1 recently saved</span>");
      expect(html).toContain("<span data-i18n-en=\"1 for this session\" data-i18n-zh=\"1 条本次临时\">1 for this session</span>");
      ...
      expect(html).toContain("<p class=\"glance-chart-insight\" data-i18n-en=\"33% long-term · 67% saved for later\" data-i18n-zh=\"33% 长期记住 · 67% 稍后整理\">33% long-term · 67% saved for later</p>");
      ...
      expect(html).toContain("<strong data-i18n-en=\"Long-term\" data-i18n-zh=\"长期记住\">Long-term</strong>");
      ...
      expect(html).toContain("<strong data-i18n-en=\"Recently saved\" data-i18n-zh=\"最近保存\">Recently saved</strong>");
      ...
      expect(html).toContain("<strong data-i18n-en=\"For this session\" data-i18n-zh=\"本次临时\">For this session</strong>");
      ...
      expect(html).toContain("<strong data-i18n-en=\"Kept for history\" data-i18n-zh=\"留作记录\">Kept for history</strong>");
      ...
      expect(detailHtml).toContain("<article><span data-i18n-en=\"Status\" data-i18n-zh=\"状态\">Status</span><strong data-memory-explorer-summary-state data-i18n-en=\"Recently saved\" data-i18n-zh=\"最近保存\">Recently saved</strong></article>");
      ...
      expect(detailHtml).toContain("<dd data-memory-explorer-detail-state data-i18n-en=\"Recently saved\" data-i18n-zh=\"最近保存\">Recently saved</dd>");
      ...
      expect(html).toContain("data-memory-explorer-next-step=\"Can be organized\"");
      ...
      expect(html).toContain("data-memory-explorer-state-en=\"Recently saved\"");
      ...
      expect(html).toContain("<span data-i18n-en=\"Kept for history\" data-i18n-zh=\"留作记录\">Kept for history</span>");
  ...
  it("expands additional stored content from the first-screen view-more control", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(serverHtml).toContain("writeStoredContentState({ overflowOpen: true, storedContentFilter: memoryStateFilter.dataset.memoryStateFilter || \"all\" });");
      expect(serverHtml).toContain("const glanceFilter = target.closest(\"[data-glance-filter]\");");
      expect(serverHtml).toContain("writeStoredContentState({ overflowOpen: true, storedContentFilter: glanceFilter.dataset.glanceFilter || \"all\" });");
      ...
      expect(serverHtml).toContain("writeStoredContentState({ storedContentFilter: storedFilter.dataset.storedContentFilter || \"all\" });");
  ...
  it("renders a local memory search for stored records and events", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(html).toContain("<option value=\"canonical\" data-i18n-en=\"Long-term\" data-i18n-zh=\"长期记住\">Long-term</option>");
      ...
      expect(html).toContain("data-memory-explorer-state-en=\"Long-term\"");
      expect(html).toContain("data-memory-explorer-state-zh=\"长期记住\"");
      ...
      expect(html).toContain("<small data-i18n-en=\"Long-term | Codex | 19d ago\" data-i18n-zh=\"长期记住 | Codex | 19 天前\">Long-term | Codex | 19d ago</small>");
      ...
      expect(html).not.toContain("data-i18n-zh=\"Long-term | Codex | 19d ago\"");
      ...
      expect(html).toContain("const hasGuidance = item.dataset.memoryExplorerHasGuidance !== \"false\" && (item.dataset.memoryExplorerWhySaved || item.dataset.memoryExplorerNextStep);");
      ...
      expect(html).toContain("setLocalizedDetailText(summaryMeaning, item.dataset.memoryExplorerNextStep || \"\", item.dataset.memoryExplorerNextStepZh || item.dataset.memoryExplorerNextStep || \"\");");
      ...
      expect(html).toContain("setLocalizedDetailText(detailMeaning, item.dataset.memoryExplorerNextStep || \"\", item.dataset.memoryExplorerNextStepZh || item.dataset.memoryExplorerNextStep || \"\");");
      expect(html).toContain("setLocalizedDetailText(detailMeaningDetail, item.dataset.memoryExplorerNextStepDetail || \"\", item.dataset.memoryExplorerNextStepDetailZh || item.dataset.memoryExplorerNextStepDetail || \"\");");
  ...
  it("supports command-style memory search shortcuts", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(html).toContain("<button type=\"button\" class=\"memory-search-chip\" data-memory-search-chip=\"state:long-term\" data-i18n-en=\"Long-term\" data-i18n-zh=\"长期记住\">Long-term</button>");
      expect(html).toContain("<button type=\"button\" class=\"memory-search-chip\" data-memory-search-chip=\"state:recently-saved\" data-i18n-en=\"Recently saved\" data-i18n-zh=\"最近保存\">Recently saved</button>");
