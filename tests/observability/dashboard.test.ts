...
describe("observability dashboard", () => {
  ...
  it("renders a human-first dashboard with memory inventory, shared copy, recent status, and language switch", async () => {
    ...
    try {
      ...
      expect(html).toContain("--canvas: #eeece7;");
      expect(html).toContain("--surface: #fbfaf7;");
      expect(html).toContain("--accent: #176b5b;");
      expect(html).toContain("--texture-line: rgba(53, 59, 55, 0.035);");
      expect(html).toContain("--panel-glow: 0 1px 2px rgba(42, 49, 45, 0.05), 0 14px 34px rgba(42, 49, 45, 0.07);");
      expect(html).toContain("--elevation-card: 0 1px 2px rgba(42, 49, 45, 0.05), 0 10px 28px rgba(42, 49, 45, 0.06);");
      expect(html).toContain("--surface-glass: rgba(251, 250, 247, 0.92);");
      ...
      expect(html).toContain("details.panel[open] > .dashboard-fold-summary {");
      expect(html).toContain("window.applyDashboardDisclosureDefaults");
      expect(html).toContain("pointer-events: none;");
      expect(html).toContain('"details.quiet-flow-details"');
      expect(html).toContain('"details.maintenance-review-summary"');
      expect(html).not.toContain('"details.maintenance-raw-overflow"');
      ...
      expect(html).toContain("<button type=\"button\" class=\"memory-state-filter memory-state-to-organize\" data-memory-state-filter=\"candidate\" data-action-board-target=\"stored-content\" aria-controls=\"stored-content\" aria-label=\"Saved for later: Saved and searchable; organize later only if useful.\" title=\"Saved for later: Saved and searchable; organize later only if useful.\" data-i18n-aria-label-en=\"Saved for later: Saved and searchable; organize later only if useful.\" data-i18n-aria-label-zh=\"已保存，稍后整理：已保存并可搜索；有用时再整理。\" data-i18n-title-en=\"Saved for later: Saved and searchable; organize later only if useful.\" data-i18n-title-zh=\"已保存，稍后整理：已保存并可搜索；有用时再整理。\">");
      expect(html).toContain("<button type=\"button\" class=\"memory-state-filter memory-state-remembered\" data-memory-state-filter=\"canonical\" data-action-board-target=\"stored-content\" aria-controls=\"stored-content\" aria-label=\"Ready to use: Moryn can use these as long-term memory.\" title=\"Ready to use: Moryn can use these as long-term memory.\" data-i18n-aria-label-en=\"Ready to use: Moryn can use these as long-term memory.\" data-i18n-aria-label-zh=\"可直接使用：Moryn 可以把这些作为长期记忆使用。\" data-i18n-title-en=\"Ready to use: Moryn can use these as long-term memory.\" data-i18n-title-zh=\"可直接使用：Moryn 可以把这些作为长期记忆使用。\">");
  ...
  it("aggregates existing reports into Governance Hub items", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(dogfoodPanelSummary).toContain("<span data-i18n-en=\"Dogfood Notes\" data-i18n-zh=\"产品记录\">Dogfood Notes</span>");
      expect(dogfoodPanelSummary).toContain("<small data-i18n-en=\"Read-only notes\" data-i18n-zh=\"只读记录\">Read-only notes</small>");
      expect(dogfoodPanelSummary).not.toContain("<span>Dogfood Notes</span>");
      expect(dogfoodPanelSummary).not.toContain("<small>Read-only notes</small>");
      ...
      expect(dogfoodPanelHtml).toContain("<strong class=\"warning\" data-i18n-en=\"Note\" data-i18n-zh=\"记录\">Note</strong>");
      expect(dogfoodPanelHtml).toContain("<span data-i18n-en=\"Read-only\" data-i18n-zh=\"只读\">Read-only</span>");
      ...
      expect(dogfoodPanelHtml).toContain("<strong data-i18n-en=\"Dogfood Notes Index\" data-i18n-zh=\"产品记录索引\">Dogfood Notes Index</strong>");
      expect(dogfoodPanelHtml).toContain("<span data-i18n-en=\"2 findings indexed\" data-i18n-zh=\"2 条产品记录已建立索引\">2 findings indexed</span>");
      expect(dogfoodPanelHtml).not.toContain("<strong>Dogfood Notes Index</strong>");
      expect(dogfoodPanelHtml).not.toContain("<span>2 findings indexed</span>");
