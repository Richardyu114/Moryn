...
describe("observability dashboard", () => {
  ...
  it("collapses Governance Hub when it only contains safe inspections", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(html).toContain("<small>inspect_failure_signals | Read-only</small>");
  ...
  it("aggregates existing reports into Governance Hub items", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(html).toContain("<span>Review Evidence</span>");
      expect(html).toContain("<span>Background Evidence</span>");
      ...
      expect(html).toContain("<span>dogfood_report</span>");
      expect(html).toContain("<small>inspect_failure_signals | Read-only</small>");
