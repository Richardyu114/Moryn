...
describe("observability dashboard", () => {
  ...
  it("reports configured dirty sync as Sync Pending instead of generic review", async () => {
    ...
    try {
      ...
      const telemetryStart = storeSignalsHtml.indexOf("data-dashboard-detail=\"store-telemetry-context\"");
      expect(syncDetailsStart).toBeGreaterThan(-1);
      expect(telemetryStart).toBeGreaterThan(syncDetailsStart);
      const focusHtml = storeSignalsHtml.slice(0, syncDetailsStart);
      const syncDetailsHtml = storeSignalsHtml.slice(syncDetailsStart, telemetryStart);
      const telemetryHtml = storeSignalsHtml.slice(telemetryStart);
      expect(focusHtml).not.toContain("<h2>Sync Position</h2>");
      expect(syncDetailsHtml).toContain("<h2>Sync Position</h2>");
      expect(focusHtml).not.toContain("<h2>Agent Activity</h2>");
      expect(focusHtml).not.toContain("<h2>Record Quality</h2>");
      expect(focusHtml).not.toContain("<h2>Record Types</h2>");
      expect(telemetryHtml).toContain("<h2>Agent Activity</h2>");
      expect(telemetryHtml).toContain("<h2>Record Quality</h2>");
      expect(telemetryHtml).toContain("<h2>Record Types</h2>");
      expect(telemetryHtml).not.toContain("<h2>Sync Position</h2>");
  ...
  it("collapses Governance Hub when it only contains safe inspections", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(html).toContain("<p>Reference checks only</p>");
      ...
      expect(html).toContain("<small>Read-only, no writes</small>");
      ...
      const safeRowEnd = html.indexOf("</details>\n    </div>", safeRowStart);
      const safeRow = html.slice(safeRowStart, safeRowEnd);
      const safeRowNotesStart = safeRow.indexOf("<details class=\"governance-safe-notes\"");
      const safeRowIntro = safeRow.slice(0, safeRowNotesStart);
      ...
      expect(safeRow).toContain("Write boundary: read-only inspection; no memory writes.");
      expect(safeRow).toContain("Evidence source: <code>dogfood_report.findings_by_id.failure_signals</code>");
      expect(safeRowIntro).not.toContain("dogfood_report.findings_by_id.failure_signals");
  ...
  it("surfaces candidate backlog as a read-only memory doctor governance item", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(candidateTriageHtml).toContain("<h2>Candidate Backlog</h2>");
      expect(candidateTriageHtml).toContain("<p>Read-only candidate groups; promotion drafts appear as explicit decisions.</p>");
      ...
      const sessionSummaryCardStart = candidateTriageHtml.indexOf("<article class=\"candidate-triage-index-card\" data-dashboard-detail=\"candidate-triage:session_summaries\" data-candidate-triage-index=\"session_summaries\">");
      const sessionSummaryCardEnd = candidateTriageHtml.indexOf("</article>", sessionSummaryCardStart) + "</article>".length;
      const sessionSummaryCardHtml = candidateTriageHtml.slice(sessionSummaryCardStart, sessionSummaryCardEnd);
      expect(sessionSummaryCardHtml).toContain("<details class=\"candidate-triage-index-evidence\" data-dashboard-detail=\"candidate-triage-index-evidence:session_summaries\">");
      expect(sessionSummaryCardHtml).toContain("<span>Evidence path</span>");
      expect(sessionSummaryCardHtml).toContain("<small>API group index</small>");
      expect(sessionSummaryCardHtml).toContain("<code>candidate_triage.groups_by_id.session_summaries</code>");
      expect(sessionSummaryCardHtml.indexOf("<code>candidate_triage.groups_by_id.session_summaries</code>")).toBeGreaterThan(sessionSummaryCardHtml.indexOf("<span>Evidence path</span>"));
      expect(sessionSummaryCardHtml.slice(0, sessionSummaryCardHtml.indexOf("<details class=\"candidate-triage-index-evidence\""))).not.toContain("<code>candidate_triage.groups_by_id.session_summaries</code>");
  ...
  it("adds a read-only candidate triage queue for memory doctor backlog", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(decisionSummaryHtml).toContain("<span>Approve Memory</span>");
      expect(decisionSummaryHtml).toContain("<span>Append-only events</span>");
      expect(decisionSummaryHtml).toContain("<span>Active candidate guard</span>");
  ...
  it("keeps read-only candidate backlog samples out of HTML while preserving full API evidence", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(groupHtml).toContain("<details class=\"candidate-triage-index-evidence\" data-dashboard-detail=\"candidate-triage-index-evidence:needs_inspection\">");
      expect(groupHtml).toContain("<span>Evidence path</span>");
      expect(groupHtml).toContain("<small>API group index</small>");
      expect(groupHtml).toContain("<code>candidate_triage.groups_by_id.needs_inspection</code>");
      expect(groupHtml.indexOf("<code>candidate_triage.groups_by_id.needs_inspection</code>")).toBeGreaterThan(groupHtml.indexOf("<span>Evidence path</span>"));
      expect(groupHtml.slice(0, groupHtml.indexOf("<details class=\"candidate-triage-index-evidence\""))).not.toContain("<code>candidate_triage.groups_by_id.needs_inspection</code>");
  ...
  it("aggregates existing reports into Governance Hub items", async () => {
    await withTempStore(async (storePath) => {
      ...
      const memoryDoctorSafeRowEnd = html.indexOf("</details>\n    </div>", memoryDoctorSafeRowStart);
      const memoryDoctorSafeRow = html.slice(memoryDoctorSafeRowStart, memoryDoctorSafeRowEnd);
      const memoryDoctorSafeRowNotesStart = memoryDoctorSafeRow.indexOf("<details class=\"governance-safe-notes\"");
      const memoryDoctorSafeRowIntro = memoryDoctorSafeRow.slice(0, memoryDoctorSafeRowNotesStart);
      ...
      expect(memoryDoctorSafeRow).toContain("Write boundary: read-only inspection; no memory writes.");
      expect(memoryDoctorSafeRow).toContain("Evidence source: <code>memory_doctor.findings_by_id.candidate_backlog</code>");
      expect(memoryDoctorSafeRowIntro).not.toContain("memory_doctor.findings_by_id.candidate_backlog");
  ...
  it("redacts quarantined record text and escapes rendered HTML", async () => {
    await withTempStore(async (storePath) => {
      ...
      const needsInspectionCardStart = candidateTriageHtml.indexOf("<article class=\"candidate-triage-index-card\" data-dashboard-detail=\"candidate-triage:needs_inspection\" data-candidate-triage-index=\"needs_inspection\">");
      const needsInspectionCardEnd = candidateTriageHtml.indexOf("</article>", needsInspectionCardStart) + "</article>".length;
      const needsInspectionCardHtml = candidateTriageHtml.slice(needsInspectionCardStart, needsInspectionCardEnd);
      expect(needsInspectionCardHtml).toContain("<details class=\"candidate-triage-index-evidence\" data-dashboard-detail=\"candidate-triage-index-evidence:needs_inspection\">");
      expect(needsInspectionCardHtml).toContain("<span>Evidence path</span>");
      expect(needsInspectionCardHtml).toContain("<small>API group index</small>");
      expect(needsInspectionCardHtml).toContain("<code>candidate_triage.groups_by_id.needs_inspection</code>");
      expect(needsInspectionCardHtml.indexOf("<code>candidate_triage.groups_by_id.needs_inspection</code>")).toBeGreaterThan(needsInspectionCardHtml.indexOf("<span>Evidence path</span>"));
      expect(needsInspectionCardHtml.slice(0, needsInspectionCardHtml.indexOf("<details class=\"candidate-triage-index-evidence\""))).not.toContain("<code>candidate_triage.groups_by_id.needs_inspection</code>");
  ...
  it("turns marker noise candidates into an explicit Review Queue archive plan", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(html).toContain("<small>Why, write preview, evidence trace</small>");
      expect(html).toContain("<h4>Why this matters</h4>");
      expect(html).toContain("<h4>Write preview</h4>");
      expect(html).toContain("<strong>Detected</strong>");
      expect(html).toContain("<strong>Impact</strong>");
      expect(html).toContain("Candidate cleanup found smoke/e2e marker noise.");
  ...
  it("renders maintenance review queue controls", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(decisionSummaryHtml).toContain("<span>Apply Repair</span>");
      expect(decisionSummaryHtml).toContain("<span>Append-only events</span>");
      expect(decisionSummaryHtml).toContain("<span>Plan hash guard</span>");
      expect(decisionSummaryHtml).not.toContain("<span>approval required</span>");
      expect(decisionSummaryHtml).not.toContain("<span>Audit evidence</span>");
      ...
      const whyStart = html.indexOf("<section class=\"maintenance-decision-section maintenance-why\"", auditDetailsStart);
      const writePreviewStart = html.indexOf("<section class=\"maintenance-decision-section maintenance-write-preview\"", auditDetailsStart);
      ...
      expect(whyStart).toBeGreaterThan(auditDetailsStart);
      expect(writePreviewStart).toBeGreaterThan(whyStart);
      expect(planEvidenceStart).toBeGreaterThan(auditDetailsStart);
      expect(planEvidenceStart).toBeGreaterThan(writePreviewStart);
  ...
  it("adds autocapture candidates to the Capture Inbox", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(decisionSummaryHtml).toContain("<span>Group approve/reject</span>");
      expect(decisionSummaryHtml).toContain("<span>Append-only events</span>");
      expect(decisionSummaryHtml).toContain("<span>Active candidate guard</span>");
      expect(decisionSummaryHtml).not.toContain("<span>approval required</span>");
      expect(decisionSummaryHtml).not.toContain("<span>Audit evidence</span>");
