...
describe("observability dashboard", () => {
  ...
  it("renders a human-first dashboard with memory inventory, shared copy, recent status, and language switch", async () => {
    ...
    try {
      ...
      expect(detailHtml).toContain("<article><span data-i18n-en=\"Meaning\" data-i18n-zh=\"含义\">Meaning</span><strong data-memory-explorer-summary-meaning data-i18n-en=\"Organize later if useful\" data-i18n-zh=\"需要时再整理\">Organize later if useful</strong></article>");
      ...
      expect(detailHtml).toContain("<strong data-memory-explorer-detail-meaning data-i18n-en=\"Organize later if useful\" data-i18n-zh=\"需要时再整理\">Organize later if useful</strong>");
      expect(detailHtml).toContain("<small data-memory-explorer-detail-meaning-detail data-i18n-en=\"Already saved and searchable. Organize it only if it should become long-term memory.\" data-i18n-zh=\"已保存并可搜索；只有需要成为长期记忆时再整理。\">Already saved and searchable. Organize it only if it should become long-term memory.</small>");
