...
describe("observability dashboard", () => {
  ...
  it("collapses Governance Hub when it only contains safe inspections", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(referenceAuditStart).toBeGreaterThan(safeRowEnd);
      ...
      expect(referenceAuditHtml).toContain("<small>Detection, boundary, and evidence</small>");
      expect(referenceAuditHtml).toContain("<h4>Failure signals</h4>");
      expect(referenceAuditHtml).toContain("Detected: Dogfood notes surfaced product friction worth inspecting.");
      expect(referenceAuditHtml).toContain("Recommended next step: inspect_failure_signals.");
      expect(referenceAuditHtml).toContain("Write boundary: read-only inspection; no memory writes.");
      expect(referenceAuditHtml).toContain("Evidence source: <code>dogfood_report.findings_by_id.failure_signals</code>");
