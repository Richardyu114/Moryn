...
describe("observability dashboard", () => {
  ...
  it("compresses Action Board summary to active counts and all clear", async () => {
    ...
    try {
      ...
      expect(html).toContain("<span>Routine Reference</span>");
      expect(html).toContain("<details class=\"evidence-library-group evidence-library-background\" data-dashboard-detail=\"evidence-background-evidence\">");
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  });
  ...
  it("renders a compact read-only Health Check summary", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(html).toContain("<section class=\"health-check-install-trust\" aria-label=\"Install Trust\">");
      expect(html).toContain("<h4>Install Trust</h4>");
      expect(html).toContain("<strong>Safe to inspect</strong>");
      expect(html).toContain("<span>4 safe checks</span>");
      expect(html).toContain("<span>1 manual input</span>");
      expect(html).toContain("<span>No host config writes from dashboard</span>");
      const installTrustStart = html.indexOf("<section class=\"health-check-install-trust\" aria-label=\"Install Trust\">");
  ...
  it("renders safe-only Governance Hub as an API-backed index", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(html).toContain("<span>Read-only Governance</span>");
      expect(html).toContain("<small>Reference checks</small>");
      ...
      expect(html).toContain("<strong>Governance Index</strong>");
      expect(html).toContain("<span>1 read-only check indexed</span>");
      expect(html).toContain("<code>governance</code>");
  ...
  it("surfaces candidate backlog as a read-only memory doctor governance item", async () => {
    await withTempStore(async (storePath) => {
      ...
      const candidateTriageEndIndex = html.indexOf("data-dashboard-detail=\"supporting-evidence\"", candidateTriageIndex);
      ...
      expect(candidateTriageHtml).toContain("<span>Candidate Backlog</span>");
      expect(candidateTriageHtml).toContain("<small>Read-only backlog</small>");
  ...
  it("keeps read-only candidate backlog samples out of HTML while preserving full API evidence", async () => {
    await withTempStore(async (storePath) => {
      ...
      const groupStart = html.indexOf("data-dashboard-detail=\"candidate-triage\"");
      const groupEnd = html.indexOf("data-dashboard-detail=\"supporting-evidence\"", groupStart);
      const groupHtml = html.slice(groupStart, groupEnd);
      expect(groupStart).toBeGreaterThan(-1);
      expect(groupEnd).toBeGreaterThan(groupStart);
      expect(groupHtml).toContain("<article class=\"candidate-triage-reference\" data-dashboard-detail=\"candidate-triage:index\" data-candidate-triage-reference>");
  ...
  it("aggregates existing reports into Governance Hub items", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(html).toContain("<small>Checks and audit</small>");
  ...
  it("redacts quarantined record text and escapes rendered HTML", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(quietWorkLanesHtml).toContain("<em>Inspect decision surfaces</em>");
      ...
      expect(quietWorkLanesHtml).toContain("<span>Context</span>");
      expect(quietWorkLanesHtml).toContain("<strong>Context unavailable</strong>");
      expect(quietWorkLanesHtml).toContain("<em>Open handoff review</em>");
      ...
      expect(quietWorkLanesHtml).toContain("<span>Evidence</span>");
      expect(quietWorkLanesHtml).toContain("<strong>Read-only reference material</strong>");
      expect(quietWorkLanesHtml).toContain("<em>Open read-only evidence</em>");
      ...
      expect(evidenceBackgroundGroupIndex).toBeGreaterThan(evidenceListIndex);
      expect(html).not.toContain("<h3>Evidence map</h3>");
      expect(html).not.toContain("<h3>Evidence routes</h3>");
      expect(html).not.toContain("<div class=\"evidence-library-brief-grid\">");
      expect(html).not.toContain("class=\"evidence-library-route\" data-evidence-library-route=\"findings\"");
      ...
      const auditSummaryRowsIndex = html.indexOf("<div class=\"supporting-evidence-summary-list\" aria-label=\"Audit Trail summary\">", supportingEvidenceIndex);
      const fullAuditDetailsIndex = html.indexOf("<details class=\"supporting-evidence-full-details\" data-dashboard-detail=\"supporting-evidence-full-details\">", supportingEvidenceIndex);
      expect(auditSummaryRowsIndex).toBeGreaterThan(supportingEvidenceIndex);
      expect(fullAuditDetailsIndex).toBeGreaterThan(auditSummaryRowsIndex);
      const auditSummaryRowsHtml = html.slice(auditSummaryRowsIndex, fullAuditDetailsIndex);
      expect(auditSummaryRowsHtml).toContain("data-supporting-evidence-summary=\"audit-evidence\"");
      expect(auditSummaryRowsHtml).toContain("data-supporting-evidence-summary=\"store-snapshot\"");
      expect(auditSummaryRowsHtml).toContain("data-supporting-evidence-summary=\"raw-store-reference\"");
      expect(auditSummaryRowsHtml).toContain("<strong>Audit Evidence</strong>");
      expect(auditSummaryRowsHtml).toContain("<span>Clean audits and store signals</span>");
      expect(auditSummaryRowsHtml).toContain("<strong>Store Snapshot</strong>");
      expect(auditSummaryRowsHtml).toContain("<span>Store context</span>");
      expect(auditSummaryRowsHtml).toContain("<strong>Raw Store Reference</strong>");
      expect(auditSummaryRowsHtml).toContain("<span>Optional raw records</span>");
      expect(auditSummaryRowsHtml).not.toContain("<details class=\"supporting-evidence-group supporting-evidence-operational\"");
      expect(html).toContain("<span>Audit Reports</span>");
      expect(html).toContain("<small>Store signals and raw reference</small>");
      expect(html).not.toContain("<span>Full audit details</span>");
      ...
      const storeSnapshotIndex = html.indexOf(storeSnapshotPanel);
      ...
      expect(storeSnapshotIndex).toBeGreaterThan(operationalEvidenceIndex);
  ...
  it("turns marker noise candidates into an explicit Review Queue archive plan", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(html).toContain("<small>Review notes, evidence trace</small>");
      expect(html).toContain("<div class=\"maintenance-review-notes\" data-maintenance-review-notes>");
      expect(html).toContain("<h4>Review notes</h4>");
      ...
      expect(html).toContain("<dt>Safety</dt><dd>No write happens until Archive Noise; the server re-runs the dry run and checks <code>plan_hash</code>. Reject hides only this browser card.</dd>");
      expect(html).toContain("<dt>Audit</dt><dd>Raw record ids, equivalent archive commands, and <code>plan_hash</code> stay in Evidence trace.</dd>");
  ...
  it("renders maintenance review queue controls", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(html).toContain("No write happens until Apply Repair; the server re-runs the dry run and checks <code>plan_hash</code>. Reject hides only this browser card.");
      ...
      expect(html).toContain("<dt>Audit</dt><dd>Raw record ids, rollback path, equivalent CLI command, and <code>plan_hash</code> stay in Evidence trace.</dd>");
      ...
      const reviewNotesStart = html.indexOf("<div class=\"maintenance-review-notes\" data-maintenance-review-notes>", auditDetailsStart);
      ...
      expect(html).toContain("Traceable by timeline");
      expect(html).toContain("No event id returned");
  ...
  it("adds autocapture candidates to the Capture Inbox", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(html).toContain("<h4>Approval brief</h4>");
      expect(html).toContain("<dt>Change</dt><dd>Review 1 candidate</dd>");
      expect(html).toContain("<dt>Scope</dt><dd>Captured through Moryn host adapter autocapture.</dd>");
      expect(html).toContain("<dt>Guard</dt><dd>Server rechecks active candidate before writing</dd>");
      expect(html).toContain("<dt>Writes</dt><dd>Approve appends memory; Reject appends archive</dd>");
      ...
      expect(html).toContain("<dt>Change</dt><dd>Review 1 candidate</dd>");
      expect(html).toContain("<dt>Scope</dt><dd>Normal review</dd>");
      expect(html).toContain("<dt>Guard</dt><dd>Server rechecks selected group records before writing</dd>");
      expect(html).toContain("<dt>Writes</dt><dd>Approve Group appends memory; Reject Group appends archive</dd>");
  ...
  it("groups Capture Inbox candidates and exposes manual review policy with noise signals", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(codexGroupFaceHtml).toContain("<h4>Approval brief</h4>");
      expect(codexGroupFaceHtml).toContain("<dt>Change</dt><dd>Review 2 candidates</dd>");
      expect(codexGroupFaceHtml).toContain("<dt>Scope</dt><dd>Normal review</dd>");
      expect(codexGroupFaceHtml).toContain("<dt>Guard</dt><dd>Server rechecks selected group records before writing</dd>");
      expect(codexGroupFaceHtml).toContain("<dt>Writes</dt><dd>Approve Group appends memory; Reject Group appends archive</dd>");
  ...
  it("keeps auto-captured handoffs out of Capture Inbox while preserving audit evidence", async () => {
    await withTempStore(async (storePath) => {
      ...
      const capturePolicyPanelEnd = html.indexOf("<details class=\"supporting-evidence-group", capturePolicyPanelStart);
      const capturePolicyHtml = html.slice(capturePolicyPanelStart, capturePolicyPanelEnd);
      expect(capturePolicyHtml).toContain("<article class=\"capture-policy-reference\" data-dashboard-detail=\"capture-policy:index\" data-capture-policy-reference>");
      expect(capturePolicyHtml).toContain("<strong>Capture Policy Index</strong>");
      expect(capturePolicyHtml).toContain("<span>1 captured route indexed</span>");
      expect(capturePolicyHtml).toContain("<code>capture_policy</code>");
      expect(capturePolicyHtml).toContain("<p>Open <code>/api/dashboard</code> for capture policy findings, safe inspection actions, decisions, rule ids, evidence paths, and trace commands.</p>");
      expect(capturePolicyHtml).not.toContain("<div class=\"capture-policy-routing-brief\" aria-label=\"Capture policy routing brief\">");
      expect(capturePolicyHtml).not.toContain("<strong>No capture inbox work</strong>");
      expect(capturePolicyHtml).not.toContain("1 auto-captured handoff");
      expect(capturePolicyHtml).not.toContain("<div class=\"lifecycle-findings\">");
      expect(capturePolicyHtml).not.toContain("Autocapture policy retained low-risk handoffs without review.");
      ...
      expect(html).not.toContain("auto-captureds");
      expect(capturePolicyHtml).not.toContain("low_risk_handoff_auto_capture");
      expect(capturePolicyHtml).not.toContain("Codex finished setup wizard polish.");
      expect(capturePolicyHtml).not.toContain("inspect_auto_captured_handoff");
      expect(capturePolicyHtml).not.toContain("moryn timeline --record-id rec_auto_capture_1 --project-id moryn --before 3 --after 3");
      expect(capturePolicyHtml).not.toContain("<summary class=\"dashboard-fold-summary\" aria-label=\"Routing details: Read-only evidence\">");
      expect(capturePolicyHtml).not.toContain("<span>Routing details</span>");
      expect(capturePolicyHtml).not.toContain("<small>Read-only evidence</small>");
  ...
  it("rechecks stale review metadata before surfacing low-risk handoffs in Capture Inbox", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(capturePolicyHtml).toContain("<span>1 captured route indexed</span>");
      expect(capturePolicyHtml).toContain("<code>capture_policy</code>");
      expect(capturePolicyHtml).not.toContain("data-capture-policy-decision=\"rec_stale_review_handoff\"");
      expect(capturePolicyHtml).not.toContain("low_risk_handoff_auto_capture");
      expect(JSON.stringify(data.capture_policy)).toContain("low_risk_handoff_auto_capture");
    });
  });
  ...
  it("does not render Capture Policy review actions for records no longer actionable in Capture Inbox", async () => {
    await withTempStore(async (storePath) => {
      ...
      const lifecycleStart = html.indexOf("<details class=\"clean-audit-report memory-lifecycle\" data-dashboard-detail=\"memory-lifecycle-audit\"");
      const lifecycleEnd = html.indexOf("<details class=\"clean-audit-report capture-policy-audit\" data-dashboard-detail=\"capture-policy-audit\"", lifecycleStart);
      const lifecycleHtml = html.slice(lifecycleStart, lifecycleEnd);
      expect(lifecycleHtml).toContain("<article class=\"memory-lifecycle-reference\" data-dashboard-detail=\"memory-lifecycle:index\" data-memory-lifecycle-reference>");
      expect(lifecycleHtml).toContain("<strong>Memory Lifecycle Index</strong>");
      expect(lifecycleHtml).toContain("<span>No lifecycle work indexed</span>");
      expect(lifecycleHtml).toContain("<code>memory_lifecycle</code>");
      expect(lifecycleHtml).toContain("<p>Open <code>/api/dashboard</code> for lifecycle policy, record assessments, findings, and suggested actions.</p>");
      expect(lifecycleHtml).not.toContain("<summary>Lifecycle suggestions</summary>");
      expect(lifecycleHtml).not.toContain("<summary>Suggested actions</summary>");
      expect(lifecycleHtml).not.toContain("No lifecycle actions suggested.");
      expect(lifecycleHtml).not.toContain("<strong>Lifecycle Policy</strong>");
      expect(lifecycleHtml).not.toContain("default_memory_lifecycle_policy");
      expect(lifecycleHtml).not.toContain("30d stale");
      expect(lifecycleHtml).not.toContain("90d archive review");
      expect(lifecycleHtml).not.toContain("low confidence");
      ...
      const capturePolicyStart = lifecycleEnd;
      const capturePolicyEnd = html.indexOf("</details>", capturePolicyStart);
      const cleanCapturePolicyHtml = html.slice(capturePolicyStart, capturePolicyEnd);
      expect(cleanCapturePolicyHtml).toContain("<article class=\"capture-policy-reference\" data-dashboard-detail=\"capture-policy:index\" data-capture-policy-reference>");
      expect(cleanCapturePolicyHtml).toContain("<strong>Capture Policy Index</strong>");
      expect(cleanCapturePolicyHtml).toContain("<span>No capture policy work indexed</span>");
      expect(cleanCapturePolicyHtml).toContain("<code>capture_policy</code>");
      expect(cleanCapturePolicyHtml).toContain("<p>Open <code>/api/dashboard</code> for capture policy findings, safe inspection actions, decisions, rule ids, evidence paths, and trace commands.</p>");
      expect(cleanCapturePolicyHtml).not.toContain("<details class=\"lifecycle-action-details\" data-dashboard-detail=\"capture-policy:default_autocapture_policy\">");
      expect(cleanCapturePolicyHtml).not.toContain("<span>Routing details</span>");
      expect(cleanCapturePolicyHtml).not.toContain("<summary class=\"dashboard-fold-summary\" aria-label=\"Routing details: Read-only evidence\">");
      expect(cleanCapturePolicyHtml).not.toContain("<small>Read-only evidence</small>");
      expect(cleanCapturePolicyHtml).not.toContain("data-capture-policy-decision=\"rec_policy_handled\"");
      expect(cleanCapturePolicyHtml).not.toContain("Review already handled");
