...
describe("observability dashboard", () => {
  ...
  it("renders a human-first dashboard with memory inventory, shared copy, recent status, and language switch", async () => {
    ...
    try {
      ...
      expect(data.dashboard_overview.detail).toBe("1 saved, not remembered yet item and 1 temporary item are saved safely. Browse them when you want to decide what Moryn remembers long term.");
      ...
      expect(html).toContain("<p data-i18n-en=\"1 saved, not remembered yet item and 1 temporary item are saved safely. Browse them when you want to decide what Moryn remembers long term.\" data-i18n-zh=\"Moryn 已安全保存 1 条尚未记住的内容和 1 条临时内容。你想整理长期记忆时再查看。\">1 saved, not remembered yet item and 1 temporary item are saved safely. Browse them when you want to decide what Moryn remembers long term.</p>");
      ...
      expect(html).toContain("<section class=\"dashboard-priority-strip\" data-dashboard-priority-strip aria-label=\"Dashboard priorities\">");
      expect(html).toContain("<article class=\"priority-card action calm\" data-dashboard-priority=\"action\">");
      expect(html).toContain("<span data-i18n-en=\"Do I need to act?\" data-i18n-zh=\"我需要操作吗？\">Do I need to act?</span>");
      expect(html).toContain("<article class=\"priority-card memory\" data-dashboard-priority=\"memory\">");
      expect(html).toContain("<span data-i18n-en=\"What is stored?\" data-i18n-zh=\"存了什么？\">What is stored?</span>");
      expect(html).toContain("<article class=\"priority-card sync good\" data-dashboard-priority=\"sync\">");
      ...
      expect(html).toContain("<strong data-i18n-en=\"2 saved items not remembered yet\" data-i18n-zh=\"2 条已保存但尚未记住的内容\">2 saved items not remembered yet</strong>");
      ...
      expect(html).toContain("<small data-i18n-en=\"This opens saved content only. Use Memory search when you want to search across memory and events.\" data-i18n-zh=\"这里仅打开已保存内容；需要跨记忆和事件搜索时，再使用搜索记忆。\">This opens saved content only. Use Memory search when you want to search across memory and events.</small>");
      ...
      expect(html).toContain("<section id=\"stored-content\" class=\"stored-content memory-explorer\" data-stored-content data-memory-explorer aria-label=\"Memory Explorer\">");
      expect(html).toContain("<h2 data-i18n-en=\"Memory Explorer\" data-i18n-zh=\"记忆浏览器\">Memory Explorer</h2>");
      expect(html).toContain("<small data-i18n-en=\"Search, filter, and inspect saved memory without writing.\" data-i18n-zh=\"搜索、筛选并查看保存的记忆；这里不会写入。\">Search, filter, and inspect saved memory without writing.</small>");
      ...
      expect(html).toContain("<p data-memory-explorer-detail-text data-i18n-en=\"Click a saved item to see the full text, source, state, and trace commands.\" data-i18n-zh=\"点击保存内容，可查看全文、来源、状态和追踪命令。\">Click a saved item to see the full text, source, state, and trace commands.</p>");
      ...
      expect(html).toContain("<span data-i18n-en=\"Saved, not remembered yet\" data-i18n-zh=\"已保存，尚未记住\">Saved, not remembered yet</span>");
  ...
  it("renders a local memory search for stored records and events", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(html).toContain("<option value=\"canonical\">Remembered</option>");
  ...
  it("keeps compact technical details quiet when only saved notes need review", async () => {
    ...
    try {
      ...
      expect(referenceRoutesHtml).toContain("<code data-reference-library-route=\"candidate-triage\">Saved notes</code>");
      ...
      expect(referenceRoutesHtml).toContain("<strong>Saved Notes</strong>");
      expect(referenceRoutesHtml).toContain("<code data-dashboard-detail=\"candidate-triage:index\">Saved notes</code>");
  ...
  it("redacts quarantined record text and escapes rendered HTML", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(referenceIndexHtml).toContain("<strong>Reference Library Index</strong>");
      expect(referenceIndexHtml).toContain("<span>Background reports indexed</span>");
  ...
  it("serves a live dashboard that refreshes from the current store", async () => {
    await withTempStore(async (storePath) => {
      ...
      try {
        ...
        expect(page).toContain("<section id=\"stored-content\" class=\"stored-content memory-explorer\" data-stored-content data-memory-explorer aria-label=\"Memory Explorer\">");
        expect(page).toContain("<div id=\"memory-search-panel\" class=\"memory-search-panel\" data-memory-search-panel>");
        ...
        expect(refreshedFragment).toContain("<section id=\"stored-content\" class=\"stored-content memory-explorer\" data-stored-content data-memory-explorer aria-label=\"Memory Explorer\">");
        expect(refreshedFragment).toContain("<div id=\"memory-search-panel\" class=\"memory-search-panel\" data-memory-search-panel>");
