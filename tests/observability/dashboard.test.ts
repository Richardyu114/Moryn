...
describe("observability dashboard", () => {
  ...
  it("keeps candidate triage samples budgeted while preserving full API evidence", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(groupHtml).toContain("<small>Full group available in API and Raw Inspector</small>");
      expect(groupHtml).toContain("<summary>API evidence path</summary>");
      expect(groupHtml).toContain("<code>candidate_triage.groups_by_id.needs_inspection.records[]</code>");
      expect(groupHtml).not.toContain("Full group stays in <code>candidate_triage.groups_by_id.needs_inspection.records[]</code> and Raw Inspector.");
