...
describe("observability dashboard", () => {
  ...
  it("surfaces candidate backlog as a read-only memory doctor governance item", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(sessionSummaryCardHtml).not.toContain("<details class=\"candidate-triage-index-evidence\" data-dashboard-detail=\"candidate-triage-index-evidence:session_summaries\">");
      expect(sessionSummaryCardHtml).not.toContain("<span>Evidence path</span>");
      expect(sessionSummaryCardHtml).not.toContain("<small>API group index</small>");
      expect(sessionSummaryCardHtml).not.toContain("<code>candidate_triage.groups_by_id.session_summaries</code>");
  ...
  it("redacts quarantined record text and escapes rendered HTML", async () => {
    await withTempStore(async (storePath) => {
      ...
      expect(data.candidate_triage.groups_by_id.needs_inspection?.evidence_path).toBe("candidate_triage.groups_by_id.needs_inspection");
      expect(needsInspectionCardHtml).not.toContain("<details class=\"candidate-triage-index-evidence\" data-dashboard-detail=\"candidate-triage-index-evidence:needs_inspection\">");
      expect(needsInspectionCardHtml).not.toContain("<span>Evidence path</span>");
      expect(needsInspectionCardHtml).not.toContain("<small>API group index</small>");
      expect(needsInspectionCardHtml).not.toContain("<code>candidate_triage.groups_by_id.needs_inspection</code>");
