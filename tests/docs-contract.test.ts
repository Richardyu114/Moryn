...
describe("documentation contracts", () => {
  ...
  it("documents the observability dashboard server and static artifact", async () => {
    ...
    expectText(dashboard, "What is stored? -> the `At a glance` memory-state chart, `Memory Explorer`");
    expectText(dashboard, "and `What Moryn remembers`");
    expectText(dashboard, "Is sync healthy? -> `This device`, the `Shared copy` card, and the sync rail");
    expectText(dashboard, "The `Needs your decision`, `Saved for later`, `At a glance`, and `Memory Explorer` rows are visible");
    ...
    expectText(dashboard, "`Memory Explorer` combines recent saved text previews, keyword search, memory-state/source filters");
    expectText(dashboard, "a detail pane with full text, source, state, and trace commands");
    expectText(dashboard, "Visible `Memory Explorer` previews render in the live dashboard when private");
    ...
    expectText(dashboard, "opens `Memory Explorer` and filters the visible previews by that state");
    expectText(dashboard, "The Explorer filter bar can switch back to `All`");
