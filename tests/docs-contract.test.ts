...
describe("documentation contracts", () => {
  ...
  it("documents the observability dashboard server and static artifact", async () => {
    ...
    expectText(dashboard, "When sync is the active warning, Store Signals promotes into a compact current-task section");
    ...
    expectText(dashboard, "raw `plan_hash`, equivalent CLI command, rollback path, and record ids stay inside `Audit details`");
    ...
    expectText(dashboard, "`Lifecycle`, `Store Signals`, and `Store History`");
    ...
    expectText(dashboard, "`Governance signals indexed`, `Dogfood signals indexed`, `Lifecycle signals");
    expectText(dashboard, "`Store position indexed`, and `History routes indexed`");
    ...
    expectText(dashboard, "`Health check`, `Candidate backlog`,");
    expectText(dashboard, "`Dogfood report`, and `Recent records`");
    ...
    expectText(dashboard, "inside compact `Background Reference`, the same stable rows are labeled `Lifecycle`, `Store Signals`, and `Store History`");
    ...
    expectText(dashboard, "The raw `plan_hash`, equivalent CLI command, rollback path, and record ids stay inside `Audit details`");
    ...
    expect(dashboard).toContain("folded summary reads `Approval context, raw");
