...
describe("documentation contracts", () => {
  ...
  it("documents the observability dashboard server and static artifact", async () => {
    ...
    expectText(dashboard, "The first screen is arranged around four plain questions");
    expectText(dashboard, "Need action? -> `Needs attention?`, `All clear`, or `Review suggested`, plus a");
    expectText(dashboard, "visible `Needs your decision` panel when review or approval is waiting");
    expectText(dashboard, "Stored what? -> the `At a glance` memory-state chart, `Stored content` text");
    expectText(dashboard, "previews, and `What Moryn remembers`");
    expectText(dashboard, "State now? -> `This device`, `Recent status`, and recent activity bars");
    expectText(dashboard, "Shared copy? -> the `Shared copy` card and sync rail");
    ...
    expectText(dashboard, "new <- candidate records");
    expectText(dashboard, "temporary <- raw records");
    expectText(dashboard, "set aside <- archived or quarantined records");
    expectText(dashboard, "`Recent status` shows the last write time, latest source, shared-copy state, and the number of new or temporary saved items");
    ...
    expectText(dashboard, "All-clear and Review-suggested pages skip duplicate shortcut grids, work lanes, and safety chips on the first screen");
    ...
    expectText(dashboard, "When the first screen is quiet, sync-only, or review-suggested, the same read-only material uses the lighter `Technical details` shell");
    ...
    expectText(dashboard, "human labels such as `Product notes`, `New notes`, and `History`");
    ...
    expectText(dashboard, "`Health check`, `New notes`,");
