...
describe("documentation contracts", () => {
  ...
  it("documents the observability dashboard server and static artifact", async () => {
    ...
    expectText(dashboard, "Recent Value card footers include record-id context beside the readable source label");
    expectText(dashboard, "timeline and recall trace commands under a `Trace commands` fold with kind/type and record-id context instead of a generic `Details` row");
    ...
    expectText(dashboard, "`Record Index` record rows use short kind/type/source plus record-id summaries in their folded row instead of raw record text");
    ...
    expectText(dashboard, "rows use short sample labels plus record ids in their folded summaries");
