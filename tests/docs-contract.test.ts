...
describe("documentation contracts", () => {
  ...
  it("documents the observability dashboard server and static artifact", async () => {
    ...
    expectText(dashboard, "The promoted panel omits `Telemetry Context`");
    ...
    expectText(dashboard, "`Raw Store Inspector` opens with `API-backed raw evidence`");
    expectText(dashboard, "lightweight API index cards labeled `Record Index`, `Event Timeline`, and `Sync Snapshot`");
    expectText(dashboard, "mapped to `/api/dashboard.recent_records[]`, `/api/dashboard.recent_events[]`, and `/api/dashboard.sync`");
    expectText(dashboard, "The dashboard HTML does not inline raw record tables, event rows, or sync detail fields there");
    expectText(dashboard, "full raw records, event operations, trace commands, and sync metadata stay in `/api/dashboard`");
    expectText(dashboard, "This keeps the stable `debug-inspector` and `inspector:*` routes");
