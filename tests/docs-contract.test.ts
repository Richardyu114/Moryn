...
describe("documentation contracts", () => {
  ...
  it("documents the observability dashboard server and static artifact", async () => {
    ...
    expectText(dashboard, "Do I need to act? -> `Needs attention?`, `All clear`, or `Saved for later`, plus a");
    expectText(dashboard, "visible `Needs your decision` panel only when approval is waiting");
    expectText(dashboard, "What is stored? -> the `At a glance` memory-state chart, `Stored content` text");
    expectText(dashboard, "previews, and `What Moryn remembers`");
    expectText(dashboard, "Is sync healthy? -> `This device`, the `Shared copy` card, and the sync rail");
    expectText(dashboard, "The `Needs your decision`, `Saved for later`, `At a glance`, and `Stored content` rows are visible");
    ...
    expectText(dashboard, "opens `Stored content` and expands saved previews without opening `Memory search`");
    ...
    expectText(dashboard, "`Memory search` is a separate local workbench with keyword, state, and source filters");
    expectText(dashboard, "Visible `Stored content` previews render in the live dashboard when private");
    ...
    expectText(dashboard, "opens `Stored content`, expands hidden saved previews, and highlights the section");
    expectText(dashboard, "`Memory search` remains a separate button");
    expectText(dashboard, "The memory-state chart is also a local navigation control");
    expectText(dashboard, "opens `Stored content` and filters the visible previews by that state");
    expectText(dashboard, "`Stored content` filter bar can switch back to `All`");
    expectText(dashboard, "`Memory search` workbench filters across recent records and events by keyword, memory state, and source");
    ...
    expectText(dashboard, "saved, not remembered yet <- candidate records");
    expectText(dashboard, "temporary <- raw records");
    expectText(dashboard, "set aside <- archived or quarantined records");
    expectText(dashboard, "`Recent status` shows the last write time, latest source, shared-copy state, and the number of saved or temporary items waiting for later organization");
    ...
    expectText(dashboard, "the first screen moves from `Background Lanes` to compact Technical details without another generic navigation fold");
