...
describe("documentation contracts", () => {
  ...
  it("documents the observability dashboard server and static artifact", async () => {
    ...
    expectText(dashboard, "When sync is the only active warning and no explicit approval is waiting, the visible Store Signals panel moves directly after Work Lanes");
    ...
    expectText(dashboard, "Routine Diagnostics and Audit Trail are grouped behind `Routine Reference`");
    expectText(dashboard, "folded summary reads `Checks and audit`");
    ...
    expectText(dashboard, "the original evidence groups stay inside the nested `Audit Reports` section");
    ...
    expectText(dashboard, "`Record Index` uses a visible `Content` header for the expandable record body");
    expectText(dashboard, "`/api/dashboard.recent_records[]` keeps the original `text` field for agents and audit tooling");
    ...
    expect(dashboard).toContain("folded summary reads `Decision record and confirmation`");
    expect(dashboard).toContain("`Confirm notes` groups the compact `Approval checklist`");
    expectText(dashboard, "keeping the `Before approving` checklist available below that record");
    expect(dashboard).toContain("Before approving");
    expectText(dashboard, "approval surface reads like a decision checklist instead of internal logs");
