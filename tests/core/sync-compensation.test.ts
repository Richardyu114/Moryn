...
} from "../../src/core/sync-compensation.js";
...
describe("sync compensation", () => {
  it("allows one safe compensation push for current-project continuity events", () => {
    expect(
      assessSyncCompensation({
        project_id: "moryn",
        status: { configured: true, remote: "origin", sync_state: "dirty", ahead: 0, behind: 0 },
        pending_paths: ["events/2026/07/evt.jsonl"],
        pending_events: [upsert(continuityRecord("rec-checkpoint"))]
      })
    ).toEqual({
      decision: "safe_to_push",
      reason: "pending_continuity_events",
      pending_paths: ["events/2026/07/evt.jsonl"],
      continuity_record_ids: ["rec-checkpoint"]
    });
  });
  ...
  it("skips pending event writes that do not carry continuity state", () => {
    ...
    expect(
      assessSyncCompensation({
        project_id: "moryn",
        status: { configured: true, remote: "origin", sync_state: "dirty", ahead: 0, behind: 0 },
        pending_paths: ["events/a.jsonl"],
        pending_events: [upsert(memory)]
      })
    ).toMatchObject({ decision: "not_needed", reason: "no_pending_continuity_events" });
  });

  it.each([
    [
      { configured: true, remote: "origin", sync_state: "clean", ahead: 0, behind: 1 },
      ["events/a.jsonl"],
      "remote_updates_pending"
    ],
    [
      { configured: true, remote: "origin", sync_state: "conflict", ahead: 0, behind: 0 },
      ["events/a.jsonl"],
      "sync_conflict"
    ],
    [
      { configured: true, remote: "origin", sync_state: "dirty", ahead: 0, behind: 0 },
      ["events/a.jsonl", "notes.txt"],
    ...
    ).toMatchObject({ decision: "blocked", reason });
  });
  ...
  it("does not compensate a Git repository without a sync remote", () => {
    expect(
      assessSyncCompensation({
        project_id: "moryn",
        status: { configured: true, sync_state: "dirty", ahead: 0, behind: 0 },
        pending_paths: ["events/a.jsonl"],
        pending_events: [upsert(continuityRecord("rec-checkpoint"))]
      })
    ).toMatchObject({ decision: "not_needed", reason: "sync_not_configured" });
  });
  ...
  it("persists the latest compensation outcome as local-only evidence", async () => {
    ...
    try {
      await writeSyncCompensationReceipt(storePath, {
        ...
        decision: "pushed",
        reason: "pending_continuity_events",
        pending_paths: ["events/a.json"],
        continuity_record_ids: ["rec-checkpoint"]
      });
      expect(await readSyncCompensationReceipt(storePath)).toEqual({
        version: 1,
        occurred_at: "2026-07-12T00:00:00.000Z",
        project_id: "moryn",
        decision: "pushed",
