...
describe("host hook normalization", () => {
  it("normalizes Codex SessionStart input", () => {
    expect(
      normalizeHostHookEvent(
        ...
        { device_id: "device-a", occurred_at: "2026-07-11T00:00:00.000Z" }
      )
    ).toEqual({
      host: "codex",
      event: "session_start",
  ...
  it("retains transcript evidence fields from observed Claude payloads", () => {
    expect(
      normalizeHostHookEvent(
        ...
        { device_id: "device-b", occurred_at: "2026-07-11T00:00:00.000Z" }
      )
    ).toMatchObject({
      transcript_path: "/home/user/.claude/projects/repo/session.jsonl"
    });
  ...
  it("normalizes Claude Code PreCompact and compact summary aliases", () => {
    expect(
      normalizeHostHookEvent(
        ...
        { device_id: "device-b", occurred_at: "2026-07-11T00:01:00.000Z" }
      )
    ).toMatchObject({
      host: "claude",
      event: "pre_compact",
  ...
  it.each([
    ...
    const input = {
      ...
      session_id: "s",
      cwd: "/repo",
      ...(hookEvent === "UserPromptSubmit" ? { prompt: "How does release rollback work?" } : {})
    };
    expect(
      normalizeHostHookEvent("claude", input, { device_id: "d", occurred_at: "2026-07-11T00:00:00.000Z" }).event
    ).toBe(expected);
  });
  ...
  it("rejects unknown events and missing stable identity", () => {
    expect(() =>
      normalizeHostHookEvent(
        "codex",
        { hook_event_name: "BeforeTool", session_id: "s", cwd: "/repo" },
        { device_id: "d", occurred_at: "2026-07-11T00:00:00.000Z" }
      )
    ).toThrow("unsupported host hook event");
    expect(() =>
      normalizeHostHookEvent(
        "codex",
