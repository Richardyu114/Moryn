...
describe("validateSemanticConsolidationProposal", () => {
  ...
  it("enforces relationship-specific thresholds and evidence", () => {
    ...
    const evidence = record({
      id: "evidence",
      type: "evidence",
      content: { text: "Source code confirms the lifecycle behavior." }
    });

    expect(
      validateSemanticConsolidationProposal([newRecord, oldRecord], proposal({ confidence: 0.979 }))
    ).toMatchObject({ status: "rejected", reason: "below_confidence_threshold" });
    expect(
      validateSemanticConsolidationProposal(
        [newRecord, oldRecord, evidence],
        proposal({
          relationship: "revises",
          semantic_equivalence: "refinement",
          confidence: 0.97,
          material_differences: [
            { field: "wording", before: "on enter", after: "when entering", significance: "minor" }
          ],
    ...
    expect(
      validateSemanticConsolidationProposal(
        ...
        proposal({
          ...
          confidence: 0.99,
          material_differences: [
            { field: "policy", before: "manual pull", after: "automatic pull", significance: "material" }
          ],
          evidence_record_ids: [evidence.id]
        })
      )
  ...
  it("rejects missing, incompatible, private, inactive, and hidden records", () => {
    ...
    expect(validateSemanticConsolidationProposal([newRecord], proposal())).toMatchObject({
      status: "rejected",
    ...
    for (const incompatible of [
      ...
      expect(validateSemanticConsolidationProposal([newRecord, incompatible], proposal())).toMatchObject({
        status: "rejected",
    ...
    expect(
      validateSemanticConsolidationProposal([newRecord, record({ id: "old", tags: ["private"] })], proposal(), {
        include_private: true
      })
    ).toMatchObject({ status: "rejected", reason: "private_boundary" });
    expect(
      validateSemanticConsolidationProposal(
        [record({ tags: ["private"] }), record({ id: "old", tags: ["private"] })],
        proposal()
      )
    ).toMatchObject({ status: "rejected", reason: "private_boundary" });
    ...
    ).toMatchObject({ status: "rejected", reason: "inactive_record" });
    expect(
      validateSemanticConsolidationProposal(
        [newRecord, record({ id: "old", state: "quarantined", visibility: "quarantined" })],
        proposal()
      )
    ).toMatchObject({ status: "rejected", reason: "inactive_record" });
    ...
    const hidden = record({
      id: "old",
      links: [{ record_id: active.id, link_type: "duplicate_of", created_at: "2026-07-12T00:00:00.000Z" }]
    });
    expect(validateSemanticConsolidationProposal([newRecord, hidden, active], proposal())).toMatchObject({
      status: "rejected",
  ...
  it("rejects existing, contradictory, and cyclic relationships", () => {
    const existingSource = record({
      links: [{ record_id: "old", link_type: "duplicate_of", created_at: "2026-07-12T00:00:00.000Z" }]
    });
    const oldRecord = record({ id: "old" });
    expect(validateSemanticConsolidationProposal([existingSource, oldRecord], proposal())).toMatchObject({
      status: "idempotent",
    ...
    const contradictory = record({
      id: "old",
      links: [{ record_id: "new", link_type: "supersedes", created_at: "2026-07-12T00:00:00.000Z" }]
    });
    expect(
      validateSemanticConsolidationProposal(
    ...
    const first = record({
      id: "first",
      links: [{ record_id: "second", link_type: "supersedes", created_at: "2026-07-12T00:00:00.000Z" }]
    });
    const second = record({
      id: "second",
      links: [{ record_id: "third", link_type: "supersedes", created_at: "2026-07-12T00:01:00.000Z" }]
    });
  ...
  it("replays an existing duplicate link after canonical target timestamps change", () => {
    const originalSource = record({
      id: "new",
      updated_at: "2026-07-12T00:02:00.000Z",
      links: [{ record_id: "old", link_type: "duplicate_of", created_at: "2026-07-12T00:02:00.000Z" }]
    });
    const originalTarget = record({
      id: "old",
      updated_at: "2026-07-12T00:01:00.000Z",
      provenance: { method: "user-confirmed" }
    });
    expect(validateSemanticConsolidationProposal([originalSource, originalTarget], proposal())).toMatchObject({
      status: "idempotent",
      reason: "existing_relationship",
      source_record_id: "new",
      target_record_id: "old"
      ...
      material_differences: [
        {
          ...
          before: "Auto-push credentials",
          after: "Never auto-push credentials",
          significance: "material"
        }
      ],
      evidence_record_ids: [agentEvidence.id]
    });
    expect(validateSemanticConsolidationProposal([newRecord, oldRecord, agentEvidence], replacement)).toMatchObject({
      status: "rejected",
      reason: "protected_replacement_requires_user_evidence"
    });
    expect(
      validateSemanticConsolidationProposal([newRecord, oldRecord, userEvidence], {
        ...replacement,
        evidence_record_ids: [userEvidence.id]
      })
    ).toMatchObject({ status: "accepted", reason: "accepted" });
  });
});
