...
const recovery = {
  ...
  learnings: [
    {
      question: "How should summaries be generated?",
      conclusion: "Use persisted checkpoint evidence.",
  ...
  knowledge_investigations: [
    {
      resolution_id: "activation",
      question: "Is Codex activation confirmed?",
      recall_status: "knowledge_gap" as const,
      recalled_record_ids: [],
      evidence: [],
      status: "unresolved" as const,
      next_step: "Observe a runtime receipt"
    }
  ],
...
describe("session synthesis", () => {
  it("preserves an explicit host-authored summary", () => {
    expect(
      synthesizeSession({ host_summary: "Host explicitly summarized the work.", recovery_pack: recovery })
    ).toEqual(
      expect.objectContaining({
        mode: "host_authored",
        summary: "Host explicitly summarized the work.",
        source_record_ids: []
      })
    );
  });
  ...
  it("synthesizes a bounded deterministic summary from durable evidence", () => {
    ...
    expect(result).toMatchObject({
      ...
      unresolved_investigations: [
        { question: "Is Codex activation confirmed?", next_step: "Observe a runtime receipt" }
      ],
      source_record_ids: ["rec-checkpoint-1"]
    });
  ...
  it("uses an honest minimal fallback when no durable evidence exists", () => {
    expect(synthesizeSession({ current_task: "Inspect release" })).toMatchObject({
      mode: "minimal_fallback",
      summary: "Session ended for task: Inspect release.",
      current_task: "Inspect release",
      source_record_ids: []
    });
    expect(synthesizeSession({})).toMatchObject({
      mode: "minimal_fallback",
      summary: "Session ended; no durable progress evidence was available."
    });
  });
});
