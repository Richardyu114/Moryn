...
describe("logical memory relationships", () => {
  it.each(relationshipTypes)("normalizes compatible %s links", (relationship) => {
    ...
    expect(
      validateLogicalRelationship([source, target], {
        record_id: source.id,
  ...
  it("rejects self links and unknown relationships", () => {
    const source = record();
    expect(() =>
      validateLogicalRelationship([source], {
        record_id: source.id,
    ...
    expect(() =>
      validateLogicalRelationship([source, record({ id: "rec-b" })], {
        record_id: source.id,
...
describe("active logical memory view", () => {
  ...
  it("hides duplicate sources and superseded or revised targets", async () => {
    ...
    const replacement = record({
      id: "replacement",
      links: [{ record_id: "old", link_type: "supersedes", created_at: "2026-07-11T00:00:00.000Z" }]
    });
    const old = record({ id: "old" });
    const revision = record({
      id: "revision",
      links: [{ record_id: "draft", link_type: "revises", created_at: "2026-07-11T00:00:00.000Z" }]
    });
    ...
    expect(view.hidden_by_record_id).toMatchObject({
      duplicate: { relationship: "duplicate_of", active_record_id: "canonical" },
      old: { relationship: "supersedes", active_record_id: "replacement" },
      draft: { relationship: "revises", active_record_id: "revision" }
    });
  });
  ...
  it("keeps conflicts and supports visible", async () => {
    ...
    const first = record({
      id: "first",
      links: [{ record_id: "second", link_type: "conflicts_with", created_at: "2026-07-11T00:00:00.000Z" }]
    });
    const second = record({
      id: "second",
      links: [{ record_id: "third", link_type: "supports", created_at: "2026-07-11T00:00:00.000Z" }]
    });
  ...
  it("keeps cyclic logical replacements visible and reports them", async () => {
    ...
    const first = record({
      id: "first",
      links: [{ record_id: "second", link_type: "supersedes", created_at: "2026-07-11T00:00:00.000Z" }]
    });
    const second = record({
      id: "second",
      links: [{ record_id: "first", link_type: "supersedes", created_at: "2026-07-11T00:00:00.000Z" }]
    });
    ...
    expect(view.findings).toContainEqual(
      expect.objectContaining({ code: "LOGICAL_RELATIONSHIP_CYCLE", record_ids: ["first", "second"] })
    );
  });
});
