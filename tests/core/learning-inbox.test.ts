...
describe("learning inbox", () => {
  ...
  it("selects same-session pending items first and bounds the result", async () => {
    await withInitializedTempStore(async (storePath) => {
      ...
      expect(pending.slice(0, 5).map((record) => record.created_at)).toEqual(
        [...pending.slice(0, 5).map((record) => record.created_at)].sort()
      );
      expect(pending.slice(5).map((record) => record.created_at)).toEqual(
        [...pending.slice(5).map((record) => record.created_at)].sort()
      );
    });
  });
  ...
  it("marks queued items consumed with produced record and origin links idempotently", async () => {
    await withInitializedTempStore(async (storePath) => {
      ...
      const checkpoint = await engine.write({
      ...
      const learning = await engine.write({
        kind: "memory",
        type: "fact",
        scope: "project",
        project_id: "moryn",
        content: { text: "Learning" },
        source
      });
  ...
  it("orders consumption after a future-dated inbox event so replay remains valid under clock skew", async () => {
    await withInitializedTempStore(async (storePath) => {
      ...
      await consumeLearningInbox(storePath, {
        inbox_records: [queued.record],
        consumed_at: "2026-07-13T01:00:00.000Z",
        consumed_by_record_id: checkpoint.record.id,
        produced_record_ids: [learning.record.id],
        source
      });
  ...
  it("rejects project learning without project identity", async () => {
    await withInitializedTempStore(async (storePath) => {
      await expect(queueLearning(storePath, input({ project_id: undefined }))).rejects.toThrow(
        "project learning requires project_id"
      );
    });
  });
});
