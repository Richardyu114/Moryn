...
function textBlocks(value: unknown): string | undefined {
  ...
  const text = value
    .filter(
      (item) =>
        item && typeof item === "object" && !Array.isArray(item) && (item as Record<string, unknown>).type === "text"
    )
...
function codexText(value: Record<string, unknown>): { user?: string; assistant?: string } {
  if (value.type !== "event_msg" || !value.payload || typeof value.payload !== "object" || Array.isArray(value.payload))
    return {};
  ...
  if (payload.type === "task_complete" && typeof payload.last_agent_message === "string")
    return { assistant: payload.last_agent_message };
  return {};
}
...
export async function readHostTranscriptEvidence(
  input: ReadHostTranscriptEvidenceInput
): Promise<HostTranscriptEvidence> {
  ...
  try {
    ...
    return {
      status: "unavailable",
      reason: (error as NodeJS.ErrnoException).code === "ENOENT" ? "not_found" : "unreadable",
      lines_considered: 0,
      malformed_lines: 0,
      truncated: false
    };
  ...
  if (fileStat.isSymbolicLink())
    return { status: "invalid", reason: "symbolic_link", lines_considered: 0, malformed_lines: 0, truncated: false };
  if (!fileStat.isFile())
    return { status: "invalid", reason: "not_regular_file", lines_considered: 0, malformed_lines: 0, truncated: false };
  ...
  if (!roots.some((root) => insideRoot(resolvedPath, root)))
    return {
      status: "invalid",
      reason: "outside_allowed_roots",
      lines_considered: 0,
  ...
  for (const line of lines) {
    try {
      ...
      const extracted =
        input.host === "codex"
          ? codexText(value as Record<string, unknown>)
          : claudeText(value as Record<string, unknown>);
  ...
  const sensitive = [user.text, assistant.text]
    .filter((value): value is string => Boolean(value))
    .some((value) => detectSensitiveContent(value).sensitive);
  if (sensitive)
    return {
      status: "protected",
