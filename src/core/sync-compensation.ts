...
export async function writeSyncCompensationReceipt(
  storePath: string,
  input: Omit<SyncCompensationReceipt, "version">
): Promise<void> {
...
export async function readSyncCompensationReceipt(storePath: string): Promise<SyncCompensationReceipt | undefined> {
  try {
    const value = JSON.parse(
      await readFile(join(storePath, "state", "sync-compensation.json"), "utf8")
    ) as SyncCompensationReceipt;
    if (
      value.version !== 1 ||
      typeof value.occurred_at !== "string" ||
      typeof value.project_id !== "string" ||
      !Array.isArray(value.pending_paths) ||
      !Array.isArray(value.continuity_record_ids)
    )
      return undefined;
    return value;
  } catch (error) {
  ...
  return [
    ...new Set(
      events
        ...
        .filter(
          (record) =>
            record?.kind === "session_summary" && record.scope === "project" && record.project_id === projectId
        )
...
export function assessSyncCompensation(input: SyncCompensationAssessmentInput): SyncCompensationAssessment {
  ...
  const result = (
    decision: SyncCompensationAssessment["decision"],
    reason: SyncCompensationReason
  ): SyncCompensationAssessment => ({
    decision,
    reason,
    pending_paths: pendingPaths,
    continuity_record_ids: continuityIds
  });
