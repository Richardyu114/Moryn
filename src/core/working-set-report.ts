...
export function summarizeWorkingSet(
  records: MorynRecord[],
  events: MorynEvent[],
  options: WorkingSetSummaryOptions = {}
) {
  ...
  const semanticRejectedProposals = records
    .flatMap((record) => {
    ...
    .filter(
      (proposal) =>
        !excludedPrivateRecordIds.has(String(proposal.source_record_id)) &&
        !excludedPrivateRecordIds.has(String(proposal.target_record_id))
    )
    .filter(
      (proposal) =>
  ...
  return {
    ...
    semantic_equivalent_links: semanticLinks.filter(
      (event) => event.op === "link_records" && event.link_type === "duplicate_of"
    ).length,
    semantic_revision_links: semanticLinks.filter(
      (event) => event.op === "link_records" && event.link_type === "revises"
    ).length,
    semantic_superseded_links: semanticLinks.filter(
      (event) => event.op === "link_records" && event.link_type === "supersedes"
    ).length,
    semantic_conflict_links: semanticLinks.filter(
