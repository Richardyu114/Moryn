...
function objectValue(value: unknown, path: string): Record<string, any> {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error(`Invalid Codex hooks: ${path} must be an object`);
...
function entryCommand(entry: Record<string, any>): string | undefined {
  ...
  const hook = entry.hooks.find((value: unknown) => value && typeof value === "object" && !Array.isArray(value) && typeof (value as Record<string, unknown>).command === "string");
...
export function mergeCodexHooks(current: unknown | undefined, artifact: HostIntegrationArtifact): CodexHooksMergeResult {
  if (artifact.host !== "codex" || artifact.format !== "json") throw new Error("Invalid Codex hooks: expected a Codex JSON artifact");
  ...
  const generatedHooks = objectValue(objectValue(JSON.parse(artifact.content), "generated root").hooks, "generated hooks");
  ...
  return { changed: JSON.stringify(root) !== JSON.stringify(settings), settings, changed_events: changedEvents, owned_entries_removed: ownedEntriesRemoved };
}
...
export async function activateCodexHooks(input: { project_path: string; artifact: HostIntegrationArtifact }): Promise<CodexActivationResult> {
  ...
  if (existingText !== undefined) {
    try { current = JSON.parse(existingText); } catch { throw new Error(`Invalid Codex hooks JSON: ${targetPath}`); }
  }
  ...
  if (!merged.changed && existingText !== undefined) return { ...merged, created: false, backup_created: false, target_path: targetPath, previous_digest: digest(existingText), new_digest: newDigest };
  ...
  if (existingText !== undefined) {
    ...
    try {
      ...
      if (await readFile(backupPath, "utf8") !== existingText) throw new Error(`Invalid Codex hooks backup content: ${backupPath}`);
    } catch (error) {
  ...
  try { await handle.writeFile(newText, "utf8"); await handle.sync(); } finally { await handle.close(); }
  await rename(temporaryPath, targetPath);
  return { ...merged, created: existingText === undefined, backup_created: existingText !== undefined, target_path: targetPath, ...(backupPath ? { backup_path: backupPath, previous_digest: digest(existingText!) } : {}), new_digest: newDigest };
}
