...
CLI required-option, non-empty string, enum, numeric-range, and JSON-object
...
expectations, `missing_argument` names a required option that was omitted, and
`retry_with` gives the exact option plus a placeholder for the corrected value.
Agents should use those fields instead of splitting the human-readable error
message.
