...
Supported sync modes are `manual`, `session`, and `interval`. Legacy `auto` is
...
Generated CLI interfaces accept flattened contract fields and literal MCP path
...
literal paths such as `"agent.client"` and `"agent.session_id"`), plus
camelCase spellings such as `agentClient`, `agentSessionId`, `projectId`,
`currentTask`, and `syncRemote`, so hosts can reuse operation-contract fields
without hand-building an `agent` object.
...
`"source.session_id"`, and also accept camelCase spellings such as `recordId`,
`linkedRecordId`, `targetState`, `linkType`, `sourceClient`, and
`sourceSessionId`. Direct MCP `write` accepts `projectId`, `contentText`,
`contentFormat`, `sourceClient`, `sourceSessionId`, and provenance camelCase
aliases in the same normalization pass. When a caller provides multiple aliases
for the same nested field with different values, Moryn returns a structured
...
or lifecycle tools must be retried with one value. Recovery hints distinguish
nested object paths from top-level literal path keys, reporting the latter as
`"\"agent.client\""`, so agents know exactly which input to remove. Moryn also
...
agent can keep exactly one mode and retry.
...
values, numeric range mistakes, nested `source.client` mistakes on core write
and mutation tools, and boolean mistakes on fields with core contracts such as
write, revise, promote, and project-init confirmation/repair fields usually come
back with `operation_contract`, `argument_sources`, and `retry_with` instead of
a host-only validation string.
...
`record_ids`, `kinds`, `scopes`, `types`, `states`, `tags`, `files`, and `limit`,
...
`next_action` inspection, local-store continuity, authored remote requirements, retry
...
`state`, `priority`, `confidence`, `confirmed`, `source.client`, `provenance`,
...
`--text`, or `--content-json` argument, so hosts can inspect the authoritative
write contract before retrying. The write operation contract exposes
...
retry guidance. When the conflict is between `source: { client: ... }` and the
...
Direct MCP tools also reject unknown top-level arguments before they can be
silently stripped by the host schema. The error returns
`expected.kind: "known_argument"` with contract-backed `argument_sources` and a
`retry_with` argument when a likely contract argument exists, or
`expected.kind: "no_arguments"` with `retry_with.arguments: {}` for zero-argument
tools such as `sync_status`. The matching `do_not` guardrails are
`send_unknown_mcp_arguments` and `retry_with_same_unknown_argument`, so agents
can remove misspelled fields such as `projectID` instead of assuming the call's
...
state values. Direct MCP `recall` also accepts singular filter aliases
(`record_id`, `recordId`, `kind`, `scope`, `type`, `state`, `tag`, and `file`)
and normalizes them into the plural array arguments, so JavaScript-style or
CLI-style callers do not accidentally broaden a query by sending an unknown
field. Conflicting singular and plural recall aliases return
`conflicting_argument.conflict_kind: "singular_vs_plural_alias"` instead of
silently choosing one value. MCP `recall.query` shape failures likewise pass through core
...
failures also pass through core validation, including numeric values, and
...
nested-vs-literal, camelCase-vs-contract, singular-vs-plural, and
...
rejected instead of being ignored. Empty CLI agent identity flags now point at
the concrete contract argument (`agent_client`, `agent_session_id`,
`agent_model`, or `agent_device_id`) instead of the aggregate `agent` object, so
agents can retry the exact bad field. Conflicting `moryn sync` operation flags
now include `operation_contracts` for `sync_status`, `sync_push`, and `sync_pull`,
and `--message` without `--push` points at
`operations_by_id.sync_push.arguments_by_name.message`.
Empty CLI `sync init ""` remotes return
...
flattened contract argument name or literal MCP path key, so
`agent_client`/`agent_session_id` or `agent.client`/`agent.session_id` can be
passed directly and still emit `--agent`/`--session-id`. The generated
`interfaces.mcp.arguments` normalizes those same aliases back into MCP JSON, so
`agent_client` or `"agent.client"` becomes `agent.client` instead of leaking as
an unsupported top-level tool argument.
...
when a host filled flattened fields such as `content_text`/`content_format` or
recovery-hint paths such as `"content.text"`/`"content.format"`.
