...
CLI required-option, non-empty string, enum, and numeric-range validation errors
also include `recovery_hint`:
`rejected_argument` preserves the rejected option and value, `expected` carries
allowed values, non-empty constraints, or numeric bounds, `missing_argument`
names a required option that was omitted, and `retry_with` gives the exact
option plus a placeholder for the corrected value. Agents should use those
fields instead of splitting the human-readable error message.
