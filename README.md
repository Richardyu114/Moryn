...
CLI required-option, non-empty string, enum, numeric-range, JSON-object, and
choose-one validation errors also include `recovery_hint`:
`rejected_argument` preserves the rejected option and value, `expected` carries
allowed values, non-empty constraints, numeric bounds, or JSON object
expectations, `missing_argument` or `missing_one_of` names omitted inputs,
`rejected_arguments` lists conflicting inputs, and `retry_with` gives exact
options plus placeholders for corrected values. Agents should use those fields
instead of splitting the human-readable error message.
