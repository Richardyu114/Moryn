...
content-addressed backups in a permission-restricted device-local directory
outside Git worktrees, then atomically replace the host file while preserving
unrelated settings. Legacy project-local `.moryn-backups` directories are
permission-hardened and Git-ignored during activation. If such a backup was
already committed, it must still be removed from the index and, when it held a
secret, remediated in repository history. Codex requires one exceptional
host-side step: use `/hooks` once to review and trust the project hooks. Moryn
never bypasses that trust check.
...
For Claude Code and Codex, `agent enter` also performs at most one automatic
activation repair when the inspector proves the change is reversible and
Moryn-owned. An invalid, symlinked, non-regular, or otherwise unsafe settings
target is never overwritten; startup continues with degraded activation
evidence instead of blocking recall and recovery. Codex still requires the user
to review the project hook once through `/hooks`; only a real hook receipt proves
the runtime integration is active.

Runtime-bound official hooks call a device-local launcher through absolute
`/bin/sh`. Each launcher lives below a private per-user root outside every Git
worktree and is scoped by the full project id, canonical project path, canonical
Store path, and host. This prevents repository content or a colliding project
slug from changing another project's trusted executable path. The stable path
also keeps the hook command and command digest unchanged across Moryn runtime
upgrades. Activation writes the new launcher atomically before updating the
generated fragment, and the launcher executes only its recorded absolute Node
and CLI paths—never a global `moryn` from `PATH`. If the recorded Node path
disappears first, the launcher may run
the same absolute CLI through a short allowlist of system-owned absolute Node
paths after a successful CLI version preflight. If the CLI is also gone, the
hook records only a local `reason=cli_unavailable` marker and exits successfully
without fabricating lifecycle evidence; health then reports repairable
`stale_moryn_config`. Activation inspection likewise reports a missing, stale,
or unavailable launcher as repairable, allowing `agent enter` to refresh the
binding without surfacing a host Hook failure to the user.

The stable launcher is a POSIX deployment path. On native Windows, where
`/bin/sh` and POSIX permission modes are unavailable, activation retains the
direct absolute runtime invocation used by earlier releases. That compatibility
path remains functional but may require host trust review after a runtime move.

Activation owns one Moryn lifecycle integration per host and project path. On
repair it removes both the current activation identity and older Moryn
commands in that project-local host file, so a path spelling, Store, or
project-id change cannot leave two lifecycle hooks running in parallel. Only
the exact static grammar generated by official Moryn hooks is claimed;
compound, dynamic, and non-Moryn user commands remain untouched. Every changed
host file receives a content-addressed, device-private backup outside the
checkout before atomic replacement.

Both official hosts enforce a 30-second outer Hook timeout. Moryn gives the
operation a 20-second absolute budget, including stdin, project discovery,
Store configuration, leases, and Git helpers. Deadline cancellation terminates
the complete Git process group (TERM, then KILL), while lease cleanup runs in a
short independent recovery window. A 25-second process watchdog exits an
official `--host-output` invocation successfully if non-cooperative local work
still has not settled, so the host remains responsive. Hook stdin is capped at
1 MiB. When a checkpoint or status was already stored before remote sync timed
out, the append-only local result remains durable and normal sync compensation
...
Host-supplied compact summaries and assistant fallback text pass the sensitive
content boundary before persistence. A sensitive payload is replaced with
generic minimal or checkpoint-recovery evidence; only metadata declaring the
payload protected is returned, and the sensitive text is never written to the
Store.
