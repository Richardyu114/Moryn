...
- Done: CLI and MCP required-option, option-dependency, non-empty string, enum,
  ...
  for `kind`, `type`, `scope`, `project_id`, `text`, `content`, `content.text`,
  `content.format`, `--text`, and `--content-json`; the write contract now
  includes `content_text` and `content_format` metadata, so agents can fetch the
  relevant fields before retrying.
