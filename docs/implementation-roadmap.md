...
| Dashboard | Dashboard first screen answers whether the user needs to act, what Moryn remembers, recent local state, and shared-copy state. Default copy is English with a Chinese language switch; technical evidence stays folded under `Technical details` while `/api/dashboard` keeps the full machine-readable trail. | `tests/observability/dashboard.test.ts`, live `/api/dashboard`, browser fragment smoke. |
