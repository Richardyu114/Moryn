...
- Done: CLI and MCP required-option, option-dependency, non-empty string, enum,
  ...
  and MCP empty-string, unknown enum-like, or numeric range values without
  parsing SDK validation prose, because common MCP string, enum-like, and
  numeric range fields now reach the Moryn operation validator before returning
  a tool error,
