...
- Done: CLI enum and numeric-range validation errors now return structured
  `error.recovery_hint` metadata with `rejected_argument`, machine-readable
  `expected` values or bounds, and `retry_with`, so shell-based agents can
  recover from hallucinated flags such as invalid `--kind`, `--state`, or
  `--limit` values without parsing English error text.
