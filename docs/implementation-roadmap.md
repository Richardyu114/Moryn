...
| Dashboard | Dashboard first screen shows one current task, not a log dump. All-clear dashboard skips duplicate `Background Status`, quiet `Info Checks`, and visible `Background Shortcuts`; Background Lanes, Reference Library, and `/api/dashboard.action_board` keep routes available. | `tests/observability/dashboard.test.ts`, live `/api/dashboard`, browser fragment smoke. |
