...
does not hide saved-but-unorganized, ready-to-use, brief, or set-aside memory from the
...
task. That click expands hidden saved previews and highlights the section so the
user can inspect what was stored without writing anything. If Capture Inbox has real approval work, the action opens Capture
...
raw client/session ids stay in `/api/dashboard` and trace commands. Trace
commands live behind a collapsed `Trace details` fold so the detail pane starts
with meaning before audit mechanics. All of these controls are client-side and read-only.
