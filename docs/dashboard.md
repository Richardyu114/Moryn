...
write is waiting. If the state is only `Recently saved`, the visible
...
separate evidence folds. Its folded summary reads `Context and evidence`, and
it uses a compact `Approval context` block rather than a raw event stream. The
context uses `Why`, `Change`, `Guard`, and `Trace` rows to explain why the plan
exists, the user-facing change, the server-side guard, and where the audit
details live.
...
`/api/dashboard.candidate_triage` plus one visible `review_focus` line softened
for the read-only UI, such as `Audit focus: Likely noise - Inspect likely noise
before archive`. That focus is read-only routing guidance, not a write action;
the raw `Start with ...` summary, full group id, recommended next step, write
boundary, confirmation flag, and evidence path live in
...
The header keeps first-screen metadata quiet: it shows `Local memory` and a
short `Updated HH:MM UTC` timestamp instead of the full filesystem path and raw
ISO timestamp. The full store path stays in the `store-path` title attribute,
the full timestamp stays in the `time` element's `datetime` and title
attributes, and `/api/dashboard.store.path` plus
...
The memory card also shows a compact `Long-term` / `Recently saved` / `For this session`
...
by itself: when the only state is `Recently saved`, `For this session`, or
...
`Long-term`, `Recently saved`, `For this session`, or `Kept for history` opens
...
result mix: `Long-term` means Moryn can already use the item as long-term
memory, `Recently saved` means the item is saved and searchable but should only
be organized if useful, `For this session` is lookup context, and `Kept for history`
...
write. The search panel also shows a live result mix for `Long-term`,
`Recently saved`, `For this session`, `Kept for history`, and `Events`, and those counts follow
...
shows available `Recently saved`, `Long-term`, `For this session`, and
...
```text
long-term             <- canonical records
recently saved        <- candidate records
for this session      <- raw records
kept for history      <- archived or quarantined records
```

Each inventory state includes a plain-language explanation. `Recently saved`
...
such as `health`, `action_board.items_by_id.review`,
...
background overview card is also a local navigation button that reuses the same
...
`Review approvals`, `Review what changed`, `Open governance`, or `Inspect sync`, so
...
Review shortcut stays quiet with `Open checks` instead of adding a second
review task. When there are no warning or critical signals, the quiet review
shortcut also reads `Open checks` instead of `Review what changed`. The
...
show how many explicit approvals are waiting there, provide one navigation button
to the owning surface, and keep one boundary line: `Append-only, guarded in
owning surface`. It does not repeat candidate group titles, maintenance plan
titles, next action labels, write labels, active guard labels, full safety notes,
raw evidence folds, machine evidence paths, or approval controls that already
live in Capture Inbox, Review Queue, or Candidate Triage. The JSON contract keeps
those per-decision audit fields in `items[]`, including `title`, `summary`,
...
full title is still `Reference Library`, while the stable route remains
`data-dashboard-detail="evidence-library"`. Its visible summary is
content-aware: when there are findings it reads `Reference material` while the
...
`Shared copy indexed`, and `History indexed` instead of
...
confirmation or non-routine Health/Recall/Context checks are grouped first under
the stable `Review Notes` group, whose visible row reads `Review Notes` with
`Reference notes`.
...
`memory-lifecycle-audit`, `capture-policy-audit`, `recent-value`,
`inspector:records`, `inspector:events`, and `inspector:sync` without expanding
full quiet reports.
