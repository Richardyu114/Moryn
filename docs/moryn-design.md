...
fields and literal MCP path keys for nested MCP arguments, so hosts can pass
...
`"choose_one"` with `choices[]`; the aggregate `apply_to` also carries
`assignment_mode: "choose_one"`, so hosts know to select one choice and use that
...
library hosts do not need to infer helper names from command examples. Unknown
lookup hints also include up to three `suggested_matches`, each with the
candidate lookup value, canonical operation id/source, and package/CLI/MCP retry
forms in the same lookup mode, so small typos can be repaired without first
loading the full index. `OperationContractLookupError` is exported for package
hosts that want to inspect those suggestions directly. MCP
...
MCP string, enum-like, numeric range, and contract-backed boolean fields are
...
component requirements such as `event_id` and `source.device_id`, sync
requirements such as `storePath`, `remoteUrl`, `options`, and `message`, choice
...
`--content-json` CLI source, and allowed values. The `write` MCP tool also
exposes and accepts those aliases, plus literal recovery-hint paths such as
`"content.text"`, `"source.client"`, and `"provenance.derived_from"`,
...
contract-backed retry guidance. When both `source: { client: ... }` and the
...
Empty CLI `--reason` values for those same mutation commands point at the same
argument source, so agents can retry the reason from the operation contract
before record lookup runs.
...
and `arguments_by_name.<argument>` source before the core engine runs. Missing
required parser options for `revise --set`, `promote --state`, `link --type`,
`agent status --status`, and `agent finish --summary` use the same source-backed
...
arguments, an operation-contract index lookup, and `do_not` guardrails.
...
`recall search extra`, return `recovery_hint.rejected_arguments.extra_positionals`,
accepted CLI arguments and options, and `retry_with.remove_positionals` instead
of a generic parser error. Recall filter positionals such as
`recall memory decision` return a `positional_mapping` from each value to
`--kind`, `--type`, and known `--scope`/`--state` filters, plus exact CLI and
...
retry forms. Agent lifecycle positional text such as
`agent status "Working..."` and `agent finish "Done..."` maps the text to
`--status` or `--summary` with exact CLI and MCP retry forms. The same
positional-to-option hints cover mutation shortcuts such as
...
`--set`, `--state`, or `--type`. These retry forms keep `retry_with.args` as the
machine-preferred argv array and render `retry_with.cli` as a shell-safe command
line for copy/paste recovery. Option typos
...
arguments use flattened contract names or literal MCP path keys for nested MCP
payloads, such as `agent_client` or `agent.client`, the generated CLI interface
falls back to that alias and emits the corresponding flag, such as `--agent`,
while the generated MCP interface emits the nested argument, such as
`agent.client`. Action
...
flattened `content_text`/`content_format` fields or recovery-hint paths such as
`"content.text"`/`"content.format"`. Action
...
For common MCP string, enum-like, numeric range, nested source-client, and
contract-backed boolean fields, validation is routed through the Moryn operation
validator so invalid values return `recovery_hint` with `operation_contract`,
`argument_sources`, and `retry_with`. MCP protocol-level validation errors can
still be reported by the MCP host before Moryn tool logic runs for shape and
type failures.
