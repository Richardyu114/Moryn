...
write content payloads, choose-one input groups, path-assignment inputs, revise
patches, and refresh cursor formats follow the same pattern:
...
object expectations, write content object/text/format requirements, choice
rules, MCP argument requirements such as write `type` and `scope`, option
