...
parsing prose. If a caller sends more than one lookup mode, the error envelope
...
write content payloads, write metadata such as `tags` and `source.client`,
...
shape, source client requirements, choice rules, MCP argument requirements such
as write `type` and `scope`, option requirements such as `--message` requiring
`--push`, project-context requirements, path-assignment shape, revise patch
rules, or ISO datetime cursor requirements, `discover_with` names safe lookup
calls such as `project_list`, and `retry_with` contains the option/argument value
placeholder to use for the corrected retry.
