...
`expected_value` plus MCP/CLI assignments. The top-level
...
`collect.apply_to.assignment_mode`, `collect.choices[]`, each choice's
`apply_to`, and each choice's `expected_value`, so a host can follow the recipe
