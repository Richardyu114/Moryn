...
read filters, project init inputs, store path inputs, sync inputs, write core
fields such as `kind`, `type`, `scope`, and `project_id`, write content
payloads, write metadata such as `tags`, `source.client`, `state`, `priority`,
`confidence`, `confirmed`, and `provenance.*`, mutation arguments such as
...
`repair`, local store path requirements such as `storePath`, sync requirements
such as `storePath`, `remoteUrl`, `options`, and `message`, choice rules, MCP
argument requirements such as write `type` and
